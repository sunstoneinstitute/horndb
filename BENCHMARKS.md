...
| `intersect` — sorted-set intersection SIMD-vs-scalar ([#132](https://github.com/sunstoneinstitute/horndb/issues/132)) | `horndb-simd` | ≥**4×** AVX-512 / ≥**2×** NEON vs scalar (SPEC-12 NF2, acceptance #3) | **MEASURED on two x86 hosts, 2026-06-30, branch `simd/real-kernels`** — genuine wide kernels (AVX-512 `cmpeq`-mask + `compressstore`; AVX2 OR-reduced `cmpeq`+`movemask`; NEON `uint64x2`). Throughput (Gelem/s, 50%-overlap L2-resident). **hel01 (Xeon Gold 5412U, Sapphire Rapids, native-512):** n=4096 scalar **1.02** / avx2 **1.77** / avx512 **2.57**; n=16384 **1.01** / **1.75** / **2.55** → **AVX-512 ~2.5× win, AVX2 ~1.7×**. **hornbench (EPYC Zen4, double-pumped 512):** n=4096 scalar **2.16** / avx2 **2.31** / avx512 **0.87** → AVX2 ≈ parity, **AVX-512 ~2.5× *regression*** (Zen4 runs 512-bit ops as 2×256 µops; `vpcompressq` microcoded). **CORRECTION (2026-07-01): these are *microbench-only* wins and do NOT hold for the real workload.** The same-session LDBC SPB-256 A/B (below) showed AVX-512/AVX2 run at ~half scalar throughput on the actual query mix — the "AVX-512 ~2.5× win on Intel" was microbench fiction, contradicted by SPB. So dispatch no longer ships an ISA on a microbench basis: a **known-CPU table** (`cpu.rs`, SPB-derived) pins **scalar** for `intersect` on both measured hosts (Zen4, Intel SPR); representative-input calibration is the fallback for unlisted CPUs (`intersect` stays AVX2 there — SPB-neutral). NEON not yet measured. | **AMBER — genuine microbench win, but SIMD is net-harmful on the real SPB-256 workload, so the known-CPU table selects scalar on both measured hosts; kept for the microbench record. Tracked in [#132](https://github.com/sunstoneinstitute/horndb/issues/132).** |
| `lower_bound` — sorted lower-bound SIMD-vs-scalar (`crates/simd/benches/lower_bound.rs`) | `horndb-simd` | beat scalar `partition_point` | **MEASURED on two x86 hosts, 2026-06-30.** Mlookup/s — **hornbench (Zen4):** n=1024 scalar **177** / simd **76**; n=4096 **140** / **25.5**; n=16384 **78.6** / **7.2**. **hel01 (Intel SPR):** n=1024 **125** / **59**; n=4096 **104** / **20**; n=16384 **63** / **5.5**. **SIMD loses 2.3×→11× on *both* archs** — galloping + linear-SIMD-scan vs scalar binary search; the gap is algorithmic (widens with size), not uarch. `lower_bound` is the **dominant SPB-256 culprit** (below): the old L2-resident, balanced calibration input mis-picked `AVX2` on Zen4 (the gallop + linear window scan losing to scalar `partition_point` on the seek-heavy leapfrog path — a −21% SPB regression). Fixed two ways: the **known-CPU table** pins scalar on both measured hosts, and **representative calibration** (seek-sweep over a >L2 haystack) now correctly picks scalar for an unlisted CPU too. (avx512 leg == avx2: no AVX-512 kernel.) | **GREEN — scalar selected on both measured hosts (table) and for unlisted CPUs (representative calibration); SIMD is an algorithmic loss on both archs and the SPB net-harm culprit.** |
| `gather` / `filter_indices_eq` — `rdf:type` scan primitives (`crates/simd/benches/{gather,filter_indices}.rs`) | `horndb-simd` | beat scalar | **MEASURED on two x86 hosts, 2026-06-30.** `gather` (Gelem/s): Zen4 n=1024 **1.94→2.96** (~1.5×), n=16384 **1.82→2.17** (~1.2×); Intel SPR n=4096 **1.27→2.78** (~2.2×), n=16384 **1.00→1.57** (~1.6×) — `vpgatherqq` is a **win on both** (contra the may-lose hypothesis). `filter_indices_eq`: **sparse** (~1% match) ~**1.9× win** on both (Zen4 5.0→9.4, Intel 3.6→7.0); **dense** (~50%) ≈ parity (Zen4 ~0.8×, Intel ~0.95×). On the real SPB-256 workload (below) SIMD is net-harmful, so the **known-CPU table** pins **scalar for all kernels** (incl. `gather`/`filter_indices_eq`) on both measured hosts. For an unlisted CPU, **representative calibration** now picks `filter_indices_eq → Scalar` (moderate-selectivity input, previously an over-eager AVX2 pick) while `gather → Avx2` remains (a genuine, SPB-neutral win). | **GREEN — table selects scalar on both measured hosts; unlisted CPUs keep `gather` AVX2 (genuine win) and `filter_indices_eq` scalar via representative calibration.** |
| `dict_decode` — bulk inline-int decode SIMD-vs-scalar (`crates/storage/benches/dict_decode.rs`) | `horndb-storage` | ≥**4×** scalar (SPEC-12 NF4, acceptance #4) | **(pending hornbench measurement)** — bench wired (`dict_decode_inline_int/scalar` + `avx2` legs via `with_forced_isa`, the latter x86_64-only) and smoke-run locally (~3.4 Gelem/s scalar on the macOS dev workstation, not recorded). Per SPEC-12 "measure first", `Dictionary::decode_inline_ints` is the storage-local mask+cast unpack (autovectorised); a dedicated `horndb-simd` unpack primitive is added only if the hornbench scalar-vs-AVX2 ratio misses 4×. Record the ratio on `hornbench` (EPYC Zen4). | **PENDING — bench wired, awaiting hornbench numbers.** |
| `partition_scan` — `rdf:type` partition scan bandwidth (`crates/storage/benches/partition_scan.rs`) | `horndb-storage` | ≥**80% STREAM Triad** bandwidth (SPEC-12 / SPEC-02 NF2, acceptance #4) | **(pending hornbench measurement)** — bench wired (`rdf_type_partition_scan/subjects_with_object`, `Throughput::Bytes` over the object column) and smoke-run locally; `PredicatePartition::subjects_with_object` scans via `horndb_simd::filter_indices_eq` + `gather` (differential-proven equal to scalar). The ≥80% gate is the NUMA-pinned hornbench number: measure STREAM-Triad once, run `numactl --cpunodebind=0 --membind=0` scan, record GB/s + the Triad fraction. | **PENDING — bench wired, awaiting NUMA-pinned hornbench numbers.** |
...
| `agg_profile` — SPARQL aggregation smoke Q1–Q5 (`crates/sparql/examples/agg_profile.rs`) ([#128](https://github.com/sunstoneinstitute/horndb/issues/128)) | `horndb-sparql` | SPEC-07 NF1 — LDBC SPB SF3 `aggregation-qps` ≤2× GraphDB Enterprise; tracking [#128](https://github.com/sunstoneinstitute/horndb/issues/128) | **Laptop `agg_profile` 100k, directional only — not the recorded hornbench bench.** Before Slice 1 of [#128](https://github.com/sunstoneinstitute/horndb/issues/128) (pre-branch baseline): Q1 COUNT(*) ~133 ms, Q2 GROUP BY ~55 ms, Q3 join+GROUP BY ~81 ms, Q4 SUM GROUP BY ~111 ms, Q5 COUNT(DISTINCT) ~37 ms. After Slice 1 (branch `perf/id-based-slot-rows-slice1`, macOS dev workstation, 2026-06-28): Q1 **38.84 ms** (~**3.4×** faster), Q2 **37.54 ms** (~**1.5×**), Q3 **66.12 ms** (~**1.2×**), Q4 **79.91 ms** (~**1.4×**), Q5 **35.63 ms** (~**1.0×**). Q1/Q2 win is structural — group keys now on raw-id `KeyPart`s (no per-row `TermId → String` decode); `COUNT(*)` is `members.len()` (zero column access); value aggregates decode only their referenced columns per member. Q3/Q4 see smaller gains (join is still a nested loop; aggregate value columns still decoded). Q5 ~1.0× because `COUNT(DISTINCT)` must decode the one column it aggregates. **Official `aggregation-qps` now recorded (hornbench SPB-256 nightly, 2026-06-29):** HornDB **~13 → ~23 qps** post-Slice-1 (~**1.77×**; two independent nightly measurements each side), GraphDB Free ~148–154 qps — the ~12× gap narrowed to **~6.5×**. **Slice 2 (all six remaining operators — LeftJoin/Union/OrderBy/Extend/Values/PathClosure — ported to native slots; decode-adapter + `cfg(test)` legacy oracle removed; one slot runtime) landed** on branch `perf/id-based-slot-rows-slice2`. Slice 2 does **not** touch the narrow aggregation *arms* (Group/COUNT/Distinct/Filter/Join are Slice 1), so `agg_profile`'s Q1–Q5 (which use only those) are unchanged by Slice 2 — confirmed by local bisect: Q1/Q3/Q4 are flat (within ~2%) across the entire 06-29→06-30 main range. **But the real SPB "aggregation" queries are not Q1–Q5** — they wrap aggregation around heavy `OPTIONAL`/`UNION`/`ORDER BY` (query1 has 15 `OPTIONAL`s; query2/query8 6 each; query9 4), and Slice 2 rewrote exactly those operators, so it *does* move SPB aggregation-qps (see root cause below). **Re-measured on hornbench 2026-06-30 (this branch, #143/#144 present): HornDB 30.78 qps vs GraphDB Free 153.6 qps → gap ~5.0×** (down from ~6.5×). Two independent same-day nightly points agree: branch `4ce02e10` **30.78**, main `b142f00c` **30.71**; GraphDB 152.6–153.6. **#143/#144 are net-neutral on this metric** — the same-day main nightly *without* #143/#144 (`b142f00c`) measured **30.71**, statistically identical to the branch's 30.78, so the streaming runtime and COUNT-over-BGP pushdown neither help nor hurt the SPB aggregation mix (it is dominated by `GROUP BY` aggregations already native since Slice 1, not COUNT-over-bare-BGP or deep operator pipelines where streaming/pushdown would bite). **Root cause of the 23 → ~30.7 step (bisected locally 2026-06-30):** #128 **Slice 2's native-slot `LeftJoin`/`OPTIONAL` hash probe** (commit `309c2db`). `git bisect` on an `OPTIONAL`+`GROUP BY` probe over the synthetic SPB-ish graph pinned it cleanly: parent `07d84c2` **225 ms/q → `309c2db` 80 ms/q (~2.8×)**; a 2×-`OPTIONAL`+`ORDER BY` probe **743 → 555 ms (~1.3×)** at the same commit; with a smaller secondary gain from native `Extend`/`BIND` (`bca05f2`). Both are Slice 2 and landed on **main** between the 06-29 and 06-30 nightlies. The old `OPTIONAL` was a materializing nested loop; the SPB aggregation mix is `OPTIONAL`-dominated, so the hash-probe rewrite lifts its qps. (The earlier "metrics-batch / environmental" guess was **wrong** — `agg_profile` Q1–Q5 carry no `OPTIONAL`, so that instrument was blind to the changed path.) Deferred (not yet landed): probe-side join streaming, filter-aware/multi-aggregate pushdown, result streaming to HTTP. **SIMD ISA cross-host A/B (2026-07-01, same-session on both hosts — this is the SPEC-12 kernel-selection basis; do NOT compare qps across the two hosts, windows differ: Zen4 600 s, Intel 240 s):** the calibrated SIMD tip was a *regression* vs scalar on both. **Zen4 hornbench (600 s):** pre-SIMD baseline **30.6** → SIMD tip calibrated (pre-fix) **28.6** → all-AVX2 forced **28.6** → **known-CPU table (all scalar) 36.16** (+26% over the 28.6 regression, +18% over the 30.6 baseline). **Intel SPR hel01 (240 s):** calibrated **34.4**, all-AVX2 forced **17.3**, AVX-512 forced **17.4**, **table 34.4** (unchanged — Intel calibration already picked scalar `lower_bound`; the table makes it deterministic/table-sourced and avoids the all-AVX2 2× cliff). Dominant culprit: `lower_bound → AVX2` on the seek-heavy leapfrog path. See the `intersect`/`lower_bound`/`gather`+`filter_indices_eq` rows above and `docs/plans/2026-07-01-simd-cpu-table-representative-calibration.md`. | **GREEN — re-measured; gain bisected to #128 Slice-2 native LeftJoin/OPTIONAL.** aggregation-qps now **~30.8** (2026-06-30, two independent points) vs GraphDB Free ~153 → **~5.0× gap** (was ~6.5× at ~23 post-Slice-1, ~12× pre-Slice-1). The 23 → ~30.7 gain is **Slice 2's native-slot `LeftJoin`/`OPTIONAL` hash probe** (commit `309c2db`, bisected locally), not the metrics batch — the SPB aggregation queries are `OPTIONAL`-dominated. **#143/#144 (streaming + COUNT-over-BGP pushdown) are net-neutral on top** (branch 30.78 ≈ same-day main 30.71). **SIMD update (2026-07-01): the known-CPU table lifts Zen4 SPB-256 from the 28.6 SIMD regression to 36.16** (all-scalar; Intel steady at 34.4) — SIMD was net-harmful for this workload; see the SIMD kernel rows above. |
...
| Bench | Crate | Notes |
|---|---|---|
| `benches/per_tuple.rs` | `horndb-wcoj` | SPEC-03 NF1 sanity check, **now owned by SPEC-12 NF1** (target ≤2.5 ns/tuple). Real microbench wired (two-star `?x p1 ?y . ?x p2 ?y` join, throughput = output tuples so per-element time *is* ns/tuple). SIMD seek (`VecIter` SoA column + `PackedColumn` block-finish) + SIMD intersect into `BatchIter` landed (SPEC-12 F1). **Measured (hornbench, 2026-06-30, 16-core, idle): ~13.44 ms / 200K tuples = ~67 ns/tuple** — ~27× over the ≤2.5 ns target, so **NF1 is not met by this bench**. The k==2 SIMD intersect wiring left it **unchanged** (criterion A/B vs pre-merge: "no change", p=0.18): this bench arms SIMD only at depth-0 (the 50K distinct `?x`, one cheap intersection), but its cost is dominated by the depth-1 leapfrog over tiny 4–8-element runs (below the 64 SIMD threshold → scalar) plus Arrow batch materialization of 200K rows. Closing NF1 needs work on that depth-1 / materialization path, not the intersect. Record on `hornbench` only, never the laptop. |
...
- **LDBC SPB nightly — running end-to-end.** `.github/workflows/nightly.yml` brings HornDB up per-run via `crates/harness/scripts/start-engine.sh` (serving the prepared flat closure, no reasoning) and drives the SPB aggregation query mix against `/query` + `/update`, recording the **full driver report** into the trend DB — not just the headline `aggregation-qps`/`editorial-qps`/`duration-s`, but per-query `Q1..Qn` count + avg/min/max ms + errors, the editorial insert/update/delete breakdown, totals, dataset sizes, and completed query mixes (parsed by `crates/harness/src/ldbc_spb.rs`; one append-only row each per engine leg, queryable via `harness report --suite ldbc-spb-256 --metric <name>`). Validated on `hornbench` 2026-06-25 at **feasible scale** — the 512 k-triple materialized SPB closure (BBC ontologies + reference datasets + 200 k generated Creative Works), aggregation-only. **Follow-up (TASKS.md):** the true SF=0.256 (256 M-triple) dataset and editorial (CW insert/update/delete) agents; until then `editorialAgents=0` and the headline metric is `aggregation-qps`, not `editorial-qps`. **[#128](https://github.com/sunstoneinstitute/horndb/issues/128) id-based slot rows landed (Slice 1 + Slice 2).** Official nightly `aggregation-qps` (hornbench, 2026-06-29): HornDB **~13 → ~23** post-Slice-1 (~1.77×), GraphDB Free ~148–154 — the ~12× gap narrowed to **~6.5×**. Slice 2 ported the last six operators to native slots and removed the decode-adapter + legacy oracle (one slot runtime); it does not touch the narrow GROUP BY/COUNT arms, but it **did** rewrite the `OPTIONAL`/`UNION`/`ORDER BY` operators the SPB aggregation queries are dominated by. **Re-measured on hornbench 2026-06-30: HornDB ~30.8 qps (branch `4ce02e10` 30.78; same-day main `b142f00c` 30.71) vs GraphDB Free ~153 → ~5.0× gap** (was ~6.5× at ~23). The 23 → ~30.7 step was **bisected locally to Slice 2's native-slot `LeftJoin`/`OPTIONAL` hash probe (`309c2db`)** — the SPB aggregation mix is `OPTIONAL`-heavy — not the metrics batch. Streaming runtime (#143) + COUNT-over-BGP pushdown (#144) proved **net-neutral on top** of Slice 2 — the same-day main run *without* them measured 30.71, identical to the branch's 30.78.
- **A/B vs GraphDB Free — wired.** SPEC-01 F10. GraphDB **10.8.14** runs as a standing, licence-free service on `hornbench` (GraphDB 11.x hard-requires a licence even for the free tier — "No license was set"; 10.x keeps the genuine free tier), provisioned by `crates/harness/scripts/bootstrap-graphdb-free-spb.sh` and holding the same closure in a no-inference `spb` repo for an apples-to-apples query A/B. The nightly's A/B leg skips gracefully (warning, not failure) if GraphDB is unreachable. _First-light feasible-scale smoke (2026-06-25, 30 s window): HornDB **0.23** vs GraphDB Free **145.9** aggregation-qps on the 512 k-triple closure._ **Recorded A/B (2026-06-29, post-Slice-1 on main): HornDB ~23 vs GraphDB Free ~148 aggregation-qps (~6.5× gap). Re-measured A/B (2026-06-30): HornDB ~30.8 vs GraphDB Free ~153 (~5.0× gap); the #143/#144 branch and same-day main measured identically (30.78 vs 30.71), so the gain over ~23 is **#128 Slice 2's native `LeftJoin`/`OPTIONAL` (`309c2db`, bisected locally), not the streaming/pushdown work**.** The A/B leg now records reliably every run after the `start-graphdb-free.sh` self-`pkill` fix (#139) — a bare `pkill -f graphdb` was matching the start script's own command line on Linux and SIGTERM-ing it, silently skipping the GraphDB leg; narrowed to `graphdb-[0-9]`. No licence restriction on publishing GraphDB Free numbers. **Durability follow-up (TASKS.md):** make GraphDB a systemd unit so it survives a hornbench reboot.
