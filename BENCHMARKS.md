...
| `intersect` — sorted-set intersection SIMD-vs-scalar ([#132](https://github.com/sunstoneinstitute/horndb/issues/132)) | `horndb-simd` | ≥**4×** AVX-512 / ≥**2×** NEON vs scalar (SPEC-12 NF2, acceptance #3) | **MEASURED — hornbench (EPYC Zen4), 2026-06-30, branch `simd/real-kernels`.** Genuine wide kernels now shipped (AVX-512 `cmpeq`-mask + `compressstore`; AVX2 OR-reduced `cmpeq`+`movemask`; NEON `uint64x2`). Throughput (Gelem/s, 50%-overlap L2-resident runs): n=1024 scalar **2.14** / avx2 **2.26** / avx512 **0.87**; n=4096 **2.16** / **2.31** / **0.87**; n=16384 **2.08** / **2.31** / **0.87**. **Floor MISSED, badly:** AVX2 ≈ parity (~1.05×), **AVX-512 is a ~2.5× *regression*** (Zen4 `vpcompressq`/all-pairs broadcast is microcoded; scalar branch prediction on regular sorted data is hard to beat). Per SPEC-12 doctrine ("ship scalar unless it clears the floor") the production default for `intersect` should be **scalar on x86** (or AVX2-at-most; never AVX-512). NEON not yet measured (needs an aarch64 bench host). | **RED vs floor — measured, does not clear 4×; AVX-512 regresses. Action: default `intersect::resolve()` to scalar on x86 (see #132).** |
| `lower_bound` — sorted lower-bound SIMD-vs-scalar (`crates/simd/benches/lower_bound.rs`) | `horndb-simd` | beat scalar `partition_point` | **MEASURED — hornbench (EPYC Zen4), 2026-06-30.** Mlookup/s: n=1024 scalar **177** / simd **76**; n=4096 **140** / **25.5**; n=16384 **78.6** / **7.2**. **SIMD is a 2.3×→11× *regression*** — the galloping + linear-SIMD-scan loses to scalar binary search and the gap widens with size. The AVX2 kernel is auto-selected in production seek paths (`packed_column::lower_bound`, `soa::LevelColumn`), so **this is a live x86 production regression**. Action: default `lower_bound::resolve()` to scalar on x86. (avx512 leg == avx2: no AVX-512 kernel, force falls through.) | **RED — measured regression; default to scalar on x86.** |
| `gather` / `filter_indices_eq` — `rdf:type` scan primitives (`crates/simd/benches/{gather,filter_indices}.rs`) | `horndb-simd` | beat scalar | **MEASURED — hornbench (EPYC Zen4), 2026-06-30.** `gather` (Gelem/s): n=1024 scalar **1.94** / simd **2.96** (~1.5×); n=16384 **1.82** / **2.17** (~1.2×) — `vpgatherqq` is a **win** on Zen4 (contra the may-lose hypothesis). `filter_indices_eq`: **sparse** (~1% match) scalar **~5.0** / simd **~9.4** (~**1.9×** win); **dense** (~50%) scalar **~3.4** / simd **~2.7** (~0.8×, loses — set-bit extraction + per-match push dominates). Net: keep SIMD for both (the `rdf:type` scan is typically selective). Consider a selectivity gate for `filter_indices_eq`. (avx512 == avx2: no AVX-512 kernels.) | **GREEN (gather, sparse filter) / mixed (dense filter) — keep SIMD; selectivity-gate filter_indices_eq.** |
