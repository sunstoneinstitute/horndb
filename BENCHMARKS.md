...
- **LDBC SPB nightly — running end-to-end.** `.github/workflows/nightly.yml` brings HornDB up per-run via `crates/harness/scripts/start-engine.sh` (serving the prepared flat closure, no reasoning) and drives the SPB aggregation query mix against `/query` + `/update`, recording `aggregation-qps` into the trend DB (`harness report --suite ldbc-spb-256 --metric aggregation-qps`). Validated on `hornbench` 2026-06-25 at **feasible scale** — the 512 k-triple materialized SPB closure (BBC ontologies + reference datasets + 200 k generated Creative Works), aggregation-only. **Follow-up (TASKS.md):** the true SF=0.256 (256 M-triple) dataset and editorial (CW insert/update/delete) agents; until then `editorialAgents=0` and the headline metric is `aggregation-qps`, not `editorial-qps`. **Slice 1 of [#128](https://github.com/sunstoneinstitute/horndb/issues/128) (id-based slot rows) landed** on branch `perf/id-based-slot-rows-slice1` — the per-row `TermId → String` decode tax for BgpScan/Group/Distinct/Project/Filter/Join is removed (directional `agg_profile` Q1 COUNT(*): ~133 ms → ~39 ms); the official `aggregation-qps` delta will appear on the next nightly run after merge. The ~12× gap is not fully closed — streaming and planner pushdown (Slice 2 / #128) remain.
