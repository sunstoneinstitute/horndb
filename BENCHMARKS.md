...
|---|---|---|---|---|
| 4-cycle, 10⁶-edge synthetic (`benches/four_cycle.rs`) | `horndb-wcoj` | WCOJ ≥10× binary-hash | _Reference workstation (2026-05-25, dense source):_ WCOJ **3.55 s** vs binary-hash **4.07 s** → **1.15× faster**. _macOS dev workstation (2026-05-31, [#15](https://github.com/sunstoneinstitute/horndb/issues/15) compressed source):_ source footprint **144 → 19.32 B/triple (7.5× smaller)**; WCOJ **2.70 s** compressed vs **4.21 s** dense (**1.56× bandwidth win**), and WCOJ flips from **0.73×** (slower than binary-hash) on the dense source to **1.11× faster** (binary-hash 3.00 s compressed) — directionally correct but still far from ≥10×. | **RED — Stage-1 acceptance #2 not met** ([#1](https://github.com/sunstoneinstitute/horndb/issues/1)). The compressed columnar source (a wcoj-side `TripleSource`, distinct from the SPEC-02 storage warm tier) shrinks the working set 7.5× and lets WCOJ overtake binary-hash, but the ≥10× ratio is not reached by compression alone — the join shape on this synthetic graph does not produce the intermediate-result blow-up that makes WCOJ asymptotically dominate. See [#1](https://github.com/sunstoneinstitute/horndb/issues/1)/[#15](https://github.com/sunstoneinstitute/horndb/issues/15). |
