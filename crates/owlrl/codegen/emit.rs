...
pub fn emit_all(rules: &[RuleSpec]) -> TokenStream {
    ...
    for r in rules {
        ...
        if r.delegate {
            ...
            fns.push(quote! {
                ...
                pub fn #fn_ident(
                    ...
                    _delta: Option<&dyn crate::store::TripleStore>,
                ) -> crate::delta::Delta {
        ...
        let body_ident = format_ident!("{}_body", fn_ident);
        let n_atoms = r.body.len();
        let src_idents: Vec<_> = (0..n_atoms).map(|i| format_ident!("__src{}", i)).collect();
        let all_store = (0..n_atoms).map(|_| quote! { store });
        // Semi-naïve variants: variant i reads body atom i from `delta` and
        // every other atom from the full store. Their union is every body
        // instantiation that involves at least one delta triple.
        let variants = (0..n_atoms).map(|i| {
            let args = (0..n_atoms).map(|j| {
                if i == j {
                    quote! { delta }
                } else {
                    quote! { store }
            ...
            quote! { #body_ident(store, &mut out, #( #args ),*); }
        });

        fns.push(quote! {
            #[doc = concat!("Compiled OWL 2 RL rule: ", #id_str)]
            #[doc = ""]
            #[doc = "`delta = None` (naïve) joins every body atom against the full store."]
            #[doc = "`delta = Some(d)` (semi-naïve) fires one variant per body atom, with"]
            #[doc = "that atom read from `d` (the previous round's newly applied triples)."]
            #[allow(unused_variables)]
            pub fn #fn_ident(
                store: &dyn crate::store::TripleStore,
                delta: Option<&dyn crate::store::TripleStore>,
            ) -> crate::delta::Delta {
                let mut out = crate::delta::Delta::new();
                match delta {
                    None => #body_ident(store, &mut out, #( #all_store ),*),
                    Some(delta) => { #( #variants )* }
                }
                out
            }
            ...
            fn #body_ident(
                store: &dyn crate::store::TripleStore,
                out: &mut crate::delta::Delta,
                #( #src_idents: &dyn crate::store::TripleStore ),*
            ) {
    ...
    quote! {
        ...
        /// `(store, delta)`: `delta` is `None` for a naïve full-store join,
        /// or `Some(previous round's newly applied triples)` for semi-naïve.
        pub type FireFn = fn(
            &dyn crate::store::TripleStore,
            Option<&dyn crate::store::TripleStore>,
        ) -> crate::delta::Delta;
...
fn emit_step(rule: &RuleSpec, plan: &Plan, depth: usize) -> TokenStream {
    ...
    // Which store this atom iterates: the full store, or the previous
    // round's delta in the semi-naïve variant that binds this atom to it.
    let src = format_ident!("__src{}", depth);
    ...
    if depth == 0 {
        ...
        let leading_iter = match (&step.p, &step.o) {
            // Constant object (`?p rdf:type owl:SymmetricProperty`): read the
            // (p, o) object index (SPEC-15 fix #1) instead of scanning the
            // whole predicate extent and filtering on `o` inside the loop.
            // The in-loop `o` filter below stays; it is redundant here.
            (
                SlotPlan::Bound(BoundSource::Vocab(field)),
                SlotPlan::Bound(BoundSource::Vocab(obj)),
            ) => {
                let id = format_ident!("{}", field);
                let oid = format_ident!("{}", obj);
                quote! { #src.probe(None, v.#id, Some(v.#oid)) }
            }
            (SlotPlan::Bound(BoundSource::Vocab(field)), _) => {
                let id = format_ident!("{}", field);
                quote! { #src.scan_predicate(v.#id) }
            }
            (
        ...
        match &step.p {
            ...
            SlotPlan::Fresh(_) => {
                quote! {
                    for #triple_var in #src.probe_any_predicate(#s_arg, #o_arg) {
                        #intra_eq_filter
                        #bind_s
                        #bind_p
                        #bind_o
            ...
            _ => {
                ...
                quote! {
                    for #triple_var in #src.probe(#s_arg, #p_arg, #o_arg) {
                        #intra_eq_filter
                        #bind_s
                        #bind_p
                        #bind_o
