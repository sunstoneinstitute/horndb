...
- Copy-on-write snapshot isolation (SPEC-02 #19) — **DELIVERED**: `Store::snapshot()` / `StoreSnapshot` pin a stable read transaction over an immutable versioned `TierSnapshot` (see `INTEGRATION-NOTES.md`). True per-tuple-visibility MVCC remains deferred to Stage 2 (intersects SPEC-06).
