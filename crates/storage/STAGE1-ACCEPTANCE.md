...
- True per-tuple-visibility MVCC + delete path (`SPEC-25` S1 [#225](https://github.com/sunstoneinstitute/horndb/issues/225), intersects SPEC-06/SPEC-24 S6) — **delivered**: begin/end stamp columns on the copy-on-write tier, `retract_quad_batch`, version-filtered reads, compaction + pin registry, and the SPEC-24 S6 snapshot surface (see `INTEGRATION-NOTES.md`). Any row above that assumed insertion-only storage no longer holds — the store can delete. The Stage-1 substitute, copy-on-write snapshot isolation (SPEC-02 #19), remains the substrate this MVCC layer sits on: `Store::snapshot()` / `StoreSnapshot` pin a stable read transaction over an immutable versioned `TierSnapshot` (see `INTEGRATION-NOTES.md`).
