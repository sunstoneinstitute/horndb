...
impl Store {
    ...
    /// Insert (graph, s, p, o) quads (SPEC-28 S6). Thin wrapper over
    /// [`Store::apply_quads`] with no dels. Caller-supplied `GraphId`s must
    /// already have been interned via `intern_graph_uri`. Returns the number
    /// of quads newly made live: a quad already visible is a counted no-op
    /// that does not bump the version (this is new — the previous
    /// unconditional-append path always bumped the version and reported no
    /// count).
    ...
    /// Retract (graph, s, p, o) quads (SPEC-25 S1 / SPEC-28 S6). Thin wrapper
    /// over [`Store::apply_quads`] with no adds. `GraphId`s must already have
    /// been interned via `intern_graph_uri`. See [`Store::retract_triples`]
    ...
    /// Atomically apply a batch of retractions and insertions as one commit
    /// version (SPEC-28 S6): `dels` take effect before `adds`, so a del+add
    /// of the same quad within the batch ends with that quad present. See
    /// [`ApplyReport`] for the count semantics (a no-net-effect batch does
    /// not bump the version). `GraphId`s in both slices must already have
    /// been interned via `intern_graph_uri`. Del terms are looked up, not
    /// interned — a term never inserted retracts nothing (a read/delete
    /// transaction must not mutate the append-only dictionary); add terms
    /// are interned. Quad identity is lexical term equality: the
    /// dictionary's identity-preserving inline-int path means e.g.
    /// `"01"^^xsd:integer` and `"1"^^xsd:integer` are distinct quads.
    pub fn apply_quads(
        &self,
