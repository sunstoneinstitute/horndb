...
HornDB is a hybrid forward/backward-chaining RDF reasoner targeting OWL 2 RL with a SPARQL 1.1 frontend, designed for unified-memory hardware (HBM / CXL). The canonical "why" lives in `specs/SPEC-00-vision.md`; read it before making architectural changes.

## Authoritative documents

These files drive the project — keep them in mind when planning work:

- `specs/SPEC-00..09-*.md` — subsystem contracts. Each ends with **Acceptance criteria** that gate the spec.
- `plans/2026-05-24-SPEC-*.md` — the one-per-spec implementation plans the Stage-1 pass executed.
...
- Plans (`plans/2026-05-24-*.md`) are historical implementation logs of the Stage-1 dispatch; treat them as commit-message-grade context, not as a source of truth for current behaviour.
