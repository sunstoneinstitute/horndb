...
| `dict_persist` — persistent dictionary flush / reopen / base probes (SPEC-25 S2, HDB-57, `crates/storage/benches/dict_persist.rs`) | `horndb-storage` | reopen is I/O-bound (mmap + header check), not a corpus re-parse (SPEC-25 acceptance #2); on the mapped base id → term is O(1) probes and term → id O(term length) (§S2 budgets) | **Pending hornbench** — the runner was offline when PLAN-25-02 landed (2026-09-04). Run via `bench.yml` with `scripts/bench/audit-pass.sh` leg `dict_persist`: 1,000,000 synthetic LUBM-shaped IRIs (`HORNDB_DICT_PERSIST_TERMS` sizes it), reporting `flush`, `open`, and per-probe `term_to_id` / `id_to_term` against the base with no repeat cache in front. The only mapped-base evidence so far is HDB-93's structure matrix below (FST term → id 223.0 ns uncached / 28.85 ns behind the repeat cache, flat-offset id → term 0.76 ns, at 100M keys) — a microbenchmark of the structures, not of `Dictionary`. Laptop numbers are not recorded. | **PENDING** |
