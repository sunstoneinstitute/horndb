...
| `serving footprint` — RSS of a serving process, per triple, now with a **per-component split** (`bench-trainmarks --mem-only`, the `[mem]` lines; driven by `scripts/bench/footprint-split.sh`) | `horndb-sparql` + `horndb-storage` | report what a serving process costs per triple **and attribute it**: columnar partitions, dictionary (keys / terms / index), the WCOJ query source built per scope, planner stats (HDB-120; the split and the `approx_bytes()` accounting behind it are HDB-146) | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-05, commit `6b35d6e`), trainmarks xlarge (9,995,000 triples). **Scope, exactly:** one process parses the corpus once, keeps the store (dictionary + columnar partitions) and one warm query snapshot, runs the five read queries, then reads `VmRSS`. No second load, no write leg, no mutation. **Default (`VecTripleSource`): 5,833 MiB = 612.0 B/triple**, split: partitions **594 MiB (10.2%, 62.3 B/triple)**; dictionary **337 MiB (5.8%)** = keys 53 + terms 68 + **index 216** (the containers' own slot capacity — 64% of the dictionary is overhead, not content); query snapshots **686 MiB (11.8%, 72.0 B/triple)**; planner stats **0**; **unattributed 4,216 MiB (72.3%)**. With `HORNDB_DIRECT_SOURCE=1`: **5,434 MiB = 570.1 B/triple**, same store components, no query snapshot, unattributed 4,487 MiB (82.6%) — the direct source is deliberately left uncounted because its leaves can be `Arc`-clones of the partitions' own columns, so counting them would double-count. **The residual is acquired by the load, not by the queries.** RSS sampled right after the load and before any query is **5,127 MiB in both legs**, against ~931 MiB of accounted live structures at that point; the five read queries then add only 706 MiB (default) / 307 MiB (direct), and in the default leg the memoised `VecTripleSource` accounts for 686 of those 706. `VmRSS` equals `VmHWM` at every sample, so no page is ever returned to the OS. What the load leaves behind is **the driver's own whole-corpus parse batch**: `bench-trainmarks` materialises all 9,995,000 triples into one `Vec<(oxrdf::Term, oxrdf::Term, oxrdf::Term)>` before interning — **1,830 MiB of inline enum alone** (192 B per row) plus its string heap — together with the loader's interning and sort/dedupe intermediates. All of it is freed, and snmalloc keeps the arenas. | **GREEN as a measurement; the number is a load high-water mark, not a serving cost.** HDB-144 asked why this row (625 B/triple) is 4.5× the same corpus's bulk-import peak (140 B/triple). The split answers it: the two loads are not the same load. The import figure comes from `Store::load_ntriples_file`, which streams in 65,536-triple batches and never holds the corpus in owned `oxrdf` terms; this driver holds all of it at once. HornDB's own serving structures on this corpus total **1,617 MiB = 170 B/triple** (partitions + dictionary + one six-ordering query snapshot), and the other 4.2 GiB is the driver's parse peak that the allocator has not handed back. Neither the dictionary (5.8% of RSS) nor the `VecTripleSource` (11.8%) is the lever HDB-144 expected. **The row's headline number should not be read as a per-triple serving cost until the driver streams its load** — see the HDB-146 follow-ups. `Dictionary::approx_bytes()` now feeds `horndb_storage_dictionary_bytes`, so the dictionary share is measurable in a live server too. |
