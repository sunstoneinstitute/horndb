...
| `serving footprint` — RSS per served triple (`bench-trainmarks`, `[mem] serving footprint` line) | `horndb-sparql` + `horndb-storage` | report what a serving process actually costs per triple: columnar partitions **plus** dictionary **plus** the WCOJ query source built per scope (HDB-120) | **PENDING hornbench.** Measurement: `scripts/bench/trainmarks.sh` (or `bench-trainmarks --file <corpus>`) on trainmarks xlarge (9,995,000 triples); the driver prints `[mem] serving footprint: RSS <n> MiB over <t> triples = <b> B/triple` after the read queries, i.e. with the store loaded **and** a query snapshot built. Report `B/triple`, and the `HORNDB_DIRECT_SOURCE=1` A/B against the default (HDB-120's direct-partition source vs the `VecTripleSource` copy) for both footprint **and** the six query times — the direct source is opt-in precisely because a laptop smoke A/B (trainmarks medium, not recorded here) showed it 2-8x slower on warm reads, and this run decides whether the footprint win is worth that or whether the merged cursor must be optimised first. No laptop numbers are recorded here. | **PENDING** — the shape is known, the total is not. Partitions ~32 B/quad (row above) + dictionary + the per-scope `VecTripleSource`, whose columns cost **24 B/triple per materialised ordering** by construction (three `u64` columns; the `Pso` anchor is built up front and each of the other five is built on first use, so 24 B/triple at one ordering up to 144 B/triple at all six). Nothing has ever measured the sum against a real corpus. HDB-120 shipped the direct-partition source that would remove the third term, but **off by default** until this run and a query-time A/B justify it. |
