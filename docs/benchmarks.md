...
| `per_tuple` — WCOJ per-tuple overhead (`benches/per_tuple.rs`) | `horndb-wcoj` | ≤**5 ns/tuple** (SPEC-03 NF1, source of truth; the ≤2.5 ns SIMD-epic #132 figure is superseded) | Two cases (hornbench, 2026-07-20, 16-core). **Marginal hot path** `wide_4x100k` (high fan-out, the NF1 measurement): **8.41 ns/tuple** (1.683 ms / 200K) — down from ~14.4 ns pre-#237. **Descent-bound** `two_star_50k` (50k subjects × 4 rows each — a full trie descent per 4 tuples, so it measures amortized descent, not the marginal cost, and structurally cannot reach NF1): **55.5 ns/tuple** (11.10 ms / 200K), −33% vs the ~82.7 ns pre-#237 baseline. #237 landed the galloping descent (`run_end`/`seek_gallop`, replacing the wide-range bisect) + bulk leaf materialization (`push_run_chunk`, replacing per-value `push_row`); a `perf` profile of `wide_4x100k` shows the residual is the AoS→SoA input-column copy (`LevelColumn::from_aos` ~46%, `distinct_run` ~33%; materialization now ~1%). | **AMBER — materially improved (marginal 14.4 → 8.4 ns/tuple), not yet ≤5 ns.** Residual is the input-column copy; a columnar `VecTripleSource` closes it, tracked in [#239](https://github.com/sunstoneinstitute/horndb/issues/239) ([#237](https://github.com/sunstoneinstitute/horndb/issues/237)) |
