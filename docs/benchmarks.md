...
| `serving footprint` — RSS per served triple (`bench-trainmarks`, `[mem] serving footprint` line) | `horndb-sparql` + `horndb-storage` | report what a serving process actually costs per triple: columnar partitions **plus** dictionary **plus** the WCOJ query source built per scope (HDB-120) | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commit `f24fe3a`), trainmarks xlarge (9,995,000 triples) via `scripts/bench/audit-pass.sh`, RSS read after the read queries — store loaded **and** a query snapshot built. **Default (`VecTripleSource`): 10,827 MiB = 1,135.8 B/triple.** With `HORNDB_DIRECT_SOURCE=1`: 10,294 MiB = **1,080.0 B/triple**. The direct source therefore saves **55.8 B/triple (−4.9%)**. Current RSS equals peak in both legs. Note the scale: this corpus is literal-heavy synthetic e-commerce data, so the **dictionary dominates** — 1,135.8 B/triple against ~32 B/quad of columnar partitions means the term store, not the indexes, is what a serving process actually pays for. The `VecTripleSource` term (24 B/triple per materialised ordering) is a small part of the total, which is why removing it moves RSS by only ~5%. | **GREEN as a measurement — the number exists now; AMBER as a footprint.** 1,135.8 B/triple is far above SPEC-02 NF1's ≤50 B/triple, but NF1 bounds the **warm columnar tier** (measured separately at 32.08 B/quad, row above), not a whole serving process including the dictionary; this row is the process total and has no NF gate of its own. The dictionary being ~97% of it is the finding, and it redirects footprint work away from the query source. **HDB-120's default stays off** — see the direct-vs-vec A/B row below: 4.9% RSS does not buy a 1.2–6.1× warm-read regression. |
| `HORNDB_DIRECT_SOURCE` A/B — direct partition source vs the `VecTripleSource` copy (HDB-120, [#334](https://github.com/sunstoneinstitute/horndb/pull/334)) | `horndb-sparql` | decide whether the direct-partition query source flips to default-on: is it within 5% on query time, and is write-then-read better? | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commits `f24fe3a` (trainmarks) and `a17bb12` (SPB, re-run once the harness waited for `/readyz`), same-session A/B, `HORNDB_DIRECT_SOURCE` unset vs `=1`. **LDBC SPB-256 `aggregation-qps` (600 s, 4 aggregation agents): 56.60 → 13.71 qps = 4.13× slower.** trainmarks xlarge warm query times (best of 3, seconds), default → direct: `q1_count` 0.423 → 1.707 (**4.03×**), `q2_customer_orders` 1.275 → 4.282 (**3.36×**), `q3_join_3_entities` 0.862 → 1.453 (**1.69×**), `q4_optional_aggregation` 1.567 → 6.563 (**4.19×**), `q5_construct` 0.368 → 0.428 (**1.16×**), `q6_delete_insert` — the write-then-read query — 0.501 → 3.076 (**6.14×**). The **load path is unaffected**: `read_ntriples` 9.908 → 9.861 s, `write_ntriples` 3.822 → 3.778 s (both inside run-to-run spread). Footprint saving is the row above: −4.9%. | **RED — keep `HORNDB_DIRECT_SOURCE` off by default.** Neither gate is met: query time is **not** within 5% (1.16–6.14× worse, 4.13× on SPB), and write-then-read is the **worst** case, not better. The 2–8× warm-read gap the laptop smoke test predicted is confirmed on hornbench at 1.2–6.1×. A 4.9% RSS win does not pay for it. The suspected cause — `MergedIter` never arming the k=2 intersect — is untouched by this run and is HDB-145's to fix; re-run this A/B after that fix before revisiting the default. |
