...
| `serving footprint` — RSS of a serving process, per triple (`bench-trainmarks --mem-only`, `[mem] serving footprint` line) | `horndb-sparql` + `horndb-storage` | report what a serving process actually costs per triple: columnar partitions **plus** dictionary **plus** the WCOJ query source built per scope (HDB-120) | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commit `403395c`), trainmarks xlarge (9,995,000 triples) via `scripts/bench/audit-pass.sh` leg `footprint`. **Scope, exactly:** one process that parses the corpus once, keeps the store (dictionary + columnar partitions) and one warm query snapshot, runs the five read queries, then reads `VmRSS`. No second load, no write leg, no mutation. **Default (`VecTripleSource`): 5,959 MiB = 625.2 B/triple.** With `HORNDB_DIRECT_SOURCE=1`: **5,468 MiB = 573.6 B/triple.** The direct source saves **51.6 B/triple (−8.2%)**. `VmRSS` equals `VmHWM` in both legs. **What is inside those bytes is unattributed.** Nothing here measures the dictionary, the partitions or the query source separately — `Dictionary` has no memory accounting — so no split is claimed. Do **not** subtract this against the 32.08 B/quad `graph_scan` row (different corpus, and that row is the columnar partitions alone) or the ~134 B/triple loader figure further down (that is the parse pipeline's transient buffer, freed as it drains). The one comparable figure is the same corpus's bulk-import peak of 1,338 MiB in the row below, and it is **not** the same thing either: that process holds no query snapshot, loads N-Triples rather than Turtle, and is sampled at the load's peak. The gap between them is unexplained and would need a heap profile, not arithmetic. | **GREEN as a measurement; AMBER as a footprint.** 625.2 B/triple is far above SPEC-02 NF1's ≤50 B/triple, but NF1 bounds the **warm columnar tier** (measured separately at 32.08 B/quad), not a whole serving process; this row is the process total and has no NF gate of its own. An earlier version of this row reported 1,135.8 B/triple: that sample was taken after the driver had also built and dropped a second full store, and snmalloc does not return freed arenas to the OS, so it was a whole-run high-water mark. `--mem-only` (this row) removes that. **HDB-120's default stays off** — see the A/B row below: 8.2% RSS does not buy a 1.2–6.1× warm-read regression. |
