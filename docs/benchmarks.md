...
| **Serving** footprint — store + query snapshot + dictionary | no separate SPEC target; tracked so the NF1 partition figure is not read as the whole serving cost | HDB-120; see the `serving footprint` row under [Measured](#measured) |
...
| Cold-tier (HDT) footprint | ≤**6 bytes/triple** amortised | NF1; measured **5.943 B/triple** on real LUBM-10 (1,272,953 triples, 18 cold partitions) — hornbench, Linux 6.12.73, commit `4105e7c`, 2026-09-06, `scripts/bench/cold-tier.sh`. **Passes with ~1% headroom**, not the ~9% the earlier proxy suggested. That proxy — **5.440 B/triple** on a 40k synthetic LUBM-shaped corpus — measured the *snapshot* encoding (`snapshot/`, SPEC-02 F9), a different file from the cold *partition* encoding (no dictionary, global `TermId` bits, one subject-major block); it is kept below as a separate figure, not as this row's number. Warm on the same corpus is 32.000 B/triple. |
...
| Tiering write amplification | ≤**1 rewrite/tier**, ≤**2× read amp** from cold | NF4; read amp measured **0.17× / 0.19× / 0.69×** (bytes the cold scan touches over a contiguous encoded scan of the same rows) on LUBM-10 — hornbench, commit `4105e7c`, 2026-09-06. Below 1× because the cold encoding is smaller than the uncompressed contiguous form. NF4 is a *bytes* clause, so that is the verdict; wall-clock decode is tracked separately — a cold object-major read costs **~3.8–4.1×** the cold subject-major scan of the same file (HDB-186), which NF4 does not bound. |
...
| Metric | Target | Baseline |
|---|---|---|
| Single-graph update → view visible again | ≤**100 ms** on a 1k-graph corpus (SPEC-06 NF1, inherited) | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commit `f24fe3a`), `crates/sparql/benches/view_derivation.rs` on the 1,000-graph corpus (spine = 20-class `subClassOf` chain, 10 instances/graph): **5.29 ms** (criterion median; one data graph dirtied, exactly one view re-derived). **GREEN — 19× inside the 100 ms budget.** Spine template build alone: **1.33 ms** |
| Spine edit → all views converged | linear in view count for P1 (P2 makes fan-out incremental) | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commit `f24fe3a`): **0.554 s for 1,000 views (0.55 ms/view)** and **0.140 s for 250 views (0.56 ms/view)** — per-view cost flat across a 4× view count, so P1's fan-out is **linear as designed**. Cold derive of the same corpora: 0.575 s / 0.140 s. **GREEN for the P1 contract**; a 5,000-view ceiling run is still P2's (SPEC-29 acceptance 7) |
| Steady-state memory, all views clean | one spine template, no per-view engine retained | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commit `f24fe3a`): **whole-process RSS 24.9 MiB with all 1,000 views clean.** Divided by view count that is **25.5 KiB/view — an upper bound on a per-view share, not the incremental cost of adding a view**: the figure includes the process, the base store and the one spine template, and the bench does not run a second view count to separate the constant from the slope. Read as a ceiling it still settles the design question — 1,000 views fit in 25 MiB total, where 1,000 retained per-view `Engine`s could not, so only the spine template is held. **GREEN** |
...
|---|---|---|---|---|
| `cold_tier` — cold-partition footprint + read amplification (`benches/cold_tier.rs`, `scripts/bench/cold-tier.sh`) | `horndb-storage` | SPEC-02 NF1 ≤6 B/triple cold; NF4 ≤2× read amp from cold (SPEC-25 acceptance #5) | hornbench (Linux 6.12.73, commit `4105e7c`, 2026-09-06), real LUBM-10 (1,272,953 triples, 18 partitions demoted), two stores over the same corpus — one left warm, one fully demoted. **NF1: cold 5.943 B/triple** vs warm 32.000 — passes the ≤6 budget with about **1% headroom**. The 40k synthetic proxy had predicted 5.440 for a *different* encoding (the snapshot format), so real LUBM is ~9% denser than that figure implied and the margin is thin, not comfortable. **NF4: 0.17× / 0.19× / 0.69×** bytes touched over a contiguous encoded scan, for the three largest predicates — under 1× because the cold encoding is smaller than the uncompressed contiguous form. Cold subject-major scan wall clock is **1.15–1.31×** warm. Promote/demote roundtrip on the largest predicate (274,699 rows): **10.05 ms promote, 5.70 ms demote**. Separately measured and **not** an NF4 miss: a cold object-major read (`ordered_at(Pos)`) costs **3.84–4.12×** the cold subject-major scan of the same file, because the cold file carries one subject-major block and every object-major read decodes and sorts from scratch. NF4 bounds bytes read, not decode CPU. | **GREEN — NF1 and NF4 both met on real LUBM-10; NF1 margin is ~1%, so a denser corpus could cross it. Object-major decode cost tracked as HDB-186** |
...
| owlrl semi-naïve firing A/B (`horndb-bench materialize --firing naive\|semi-naive`; compiled rules fire one delta-bound variant per body atom, SPEC-15 fix #2, HDB-40) | `horndb-owlrl` | cross-round re-derivation gone; closure identical; record `reason_ms` + rounds ([#134](https://github.com/sunstoneinstitute/horndb/issues/134)) | hornbench (16-core Ryzen, Linux 6.12.73, rustc 1.90.0, 2026-09-03, commit `f32420f`, `scripts/bench/seminaive-ab.sh`, same binary both arms), taxonomy d=12 / 40 k inst (480,372 inferred, 520,384 total), median of 3 per cell. Rule-firing backend: `compiled_rules_ms` naive **212.0 → semi-naïve 208.3 ms**, `reason_ms` **506.0 → 521.6 ms**, `apply_ms` 253 → 245. GraphBLAS backend: `compiled_rules_ms` 218.0 → 216.6 ms, `reason_ms` 503.5 → 518.0 ms. **Rounds 4 → 4, inferred identical (480,372), dumped closures identical (520,384 triples).** Within noise on this corpus: `cax-sco`'s round-4 work is dominated by *intra-delta* redundancy (the 68-pair `scm-sco` closure derives the same `x rdf:type Cj` through many paths, filtered by `contains`), which semi-naïve cannot remove; the old×old joins it does remove are a small share here, and building the per-round delta `MemStore` costs ~15 ms. **LUBM-1 leg — was blocked, unblocked by HDB-147, re-run pending.** LUBM-1/LUBM-10 come from the `bench-corpora` release via `scripts/bench/get_lubm.sh` (hornbench fetched both, run 33810258305). The first attempt found **87,027,924 inferred triples from 100,866 asserted, 86,964,950 of them (99.93 %) with a *literal* subject** — exactly `L × (L + 1)` for the `L = 9,325` comparable literal terms, because `inject_datatype_literal_axioms` asserted `owl:differentFrom`/`owl:sameAs` for **every pair** of comparable literals before reasoning; one run cost 10.2 GiB peak RSS and ~38 s, and it OOM-killed the bench host. **HDB-147 fixed the injection** (`dt-eq` bucketed by canonical value, O(k); `dt-diff` derived only for literal pairs the closure makes `owl:sameAs`). On a laptop smoke run after the fix (diagnostic only, not a recorded number) LUBM-1 closes to the expected ~164 k non-literal-subject triples. The semi-naïve A/B on LUBM-1 and LUBM-10 has **not been re-measured on hornbench** yet — the runner was offline when HDB-147 landed. Follow-up: run `scripts/bench/seminaive-ab.sh` with `LUBM_N="1 10"` through `bench.yml` and fill in this row. | **AMBER — parity GREEN, wall-clock flat on the taxonomy corpus; the LUBM-1/LUBM-10 leg is unblocked (HDB-147) but still pending a hornbench run.** The claim "semi-naïve removes cross-round re-derivation" is proven by construction and by `tests/semi_naive_differential.rs` (closure **and** round count identical to the naïve oracle, five corpora + proptest + both closure backends); the claim "and that makes materialization faster" is **not** demonstrated on any corpus measured so far ([#134](https://github.com/sunstoneinstitute/horndb/issues/134)) |
...
| `stats_incremental` — does a small write make the next query rebuild `SnapshotStats`? (HDB-123, [#331](https://github.com/sunstoneinstitute/horndb/pull/331)) | `horndb-sparql` | a 1k-quad batch must not make the next plan pay an `O(store)` statistics rebuild | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commits `f24fe3a` and `c2d5df3`, two independent runs), `crates/sparql/benches/stats_incremental.rs`. The bench is deliberately bimodal, so the mean is meaningless and the raw criterion samples are read instead. `plan_only` (untouched store, the floor) is tight: min **6.87 µs**, median 6.91, p99 6.96. `plan_after_batch` splits cleanly: min **63.2 µs** (the merged-summary fast path) against p99 **2,704 µs** (the ~1-in-20 batch that exceeds the drift bound and rebuilds in full), mean ~303 µs. Merge path is therefore **9.2× the floor**, rebuild **391×** — a 43× separation between the two modes. | **GREEN — the merge path is live.** Before HDB-123 every batch dropped the cached summary, so every query paid the ~2.7 ms rebuild; now ~19 in 20 pay 63 µs. Caveat: the bench pins one store size, and its own stated failure test is whether the *fast path* scales with store size — proving that needs a second size, which this pass did not run. |
| `write_under_concurrent_reader` — write latency while a reader scans (HDB-122, [#332](https://github.com/sunstoneinstitute/horndb/pull/332)) | `horndb-storage` | a concurrent reader must not make write latency track partition size | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commits `f24fe3a` and `c2d5df3`, two independent runs), `crates/storage/benches/insert_retract.rs`, median across two runs: **10k triples 18.0 µs, 100k 17.9 µs, 1M 27.1 µs**. p99 write latency: **21.8 µs / 21.7 µs / 45.8 µs**. So both median and tail are **flat from 10k to 100k** (a 10× partition growth costs nothing) and roughly **double at 1M** (median +51%, p99 +110%). Fastest samples are 4.9-6.1 µs at every size, so the floor does not move at all — the growth is in the tail, not the cost of a write. | **GREEN — no O(partition) write path.** Latency is flat over the first 10× and sub-linear over the next; a reader-blocked writer would show the opposite. The 1M tail widening is the one thing worth watching if partitions grow further. |
| `dict_persist` — persistent dictionary flush / reopen / base probes (SPEC-25 S2, HDB-57, `crates/storage/benches/dict_persist.rs`) | `horndb-storage` | SPEC-25 acceptance #2: a store holding the **full LUBM-100 dictionary** closed and reopened from disk resolves id → term and term → id (plus at least one named graph's `GraphId`) without re-interning, and reopen time is I/O-bound (mmap + header check), not a corpus re-parse; on the mapped base id → term is O(1) probes and term → id O(term length) (§S2 budgets); the HDB-91 amendment (reopen-and-append 10% through the bulk loader, dictionary share 9–12%) grades the dictionary's worth | **Pending hornbench** — the runner was offline when PLAN-25-02 landed (2026-09-04). Two measurements are owed: (1) the LUBM-100 reopen itself — load LUBM-100, `Store::flush_dictionary`, time `Dictionary::open`, then probe every term both ways; not yet a bench leg, run it by hand from the `load_lubm` data path and record it here; (2) the `dict_persist` leg via `bench.yml` (`scripts/bench/audit-pass.sh`): 1,000,000 synthetic LUBM-shaped IRIs (`HORNDB_DICT_PERSIST_TERMS` sizes it), reporting `flush`, `open`, and per-probe `term_to_id` / `id_to_term` against the base with no repeat cache in front — a proxy for the probe budgets, not the acceptance run. The only mapped-base evidence so far is HDB-93's structure matrix below (FST term → id 223.0 ns uncached / 28.85 ns behind the repeat cache, flat-offset id → term 0.76 ns, at 100M keys) — a microbenchmark of the structures, not of `Dictionary`. Laptop numbers are not recorded. | **PENDING** |
| `wal` — write-ahead log append under each fsync policy, replay on open (SPEC-25 S3, HDB-58, `crates/storage/benches/wal_append.rs`) | `horndb-storage` | SPEC-25 acceptance #3 has no numeric target; the leg grades the S3 tax: `append/every_batch` (fsync per batch) and `append/timed` (`SyncPolicy::Every(1s)`) time `Store::insert_quads` of one 10,000-quad batch with fresh terms (`HORNDB_WAL_BATCH` sizes it), `replay/open` times `Store::open` over 50 such batches (`HORNDB_WAL_REPLAY_BATCHES`) | **Pending hornbench** — the runner was offline when PLAN-25-03 landed (2026-09-04). Run the `wal` leg via `bench.yml` (`scripts/bench/audit-pass.sh`) and record append cost per batch under both policies next to the in-memory `insert_quads` cost, plus replay throughput. Laptop numbers are not recorded. | **PENDING** |
| `dict_gc_churn` — dictionary churn with and without compaction (HDB-121, [#333](https://github.com/sunstoneinstitute/horndb/pull/333)) | `horndb-storage` | compaction must not cost throughput; RSS must plateau under churn | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commits `f24fe3a` and `c2d5df3`, two independent runs), `crates/storage/benches/dict_gc_churn.rs`, 4×1k churn cycles: **no GC 3.48-3.50 ms**, **compacting GC 3.13-3.14 ms** — compaction is **~10% faster**, not slower, and both distributions are tight (p99 within 1% of the median). | **GREEN on time, UNMEASURED on memory.** Compaction pays for itself on this workload (reclaiming slots keeps the live set denser than the churn costs to collect). The **RSS plateau the task asked for is not recorded: the bench only times, it never samples RSS**, so proving the plateau needs the bench to grow a memory gauge — filed as follow-up work, not guessed at here. |
| Closure backend selection — rule-firing vs GraphBLAS (HDB-126, [#329](https://github.com/sunstoneinstitute/horndb/pull/329)); also the HDB-117 [#328](https://github.com/sunstoneinstitute/horndb/pull/328) materialize leg | `horndb-owlrl` + `horndb-closure` | does the backend choice change end-to-end materialization? | hornbench (Ryzen 7 7700, Debian 6.12, rustc 1.90.0, 2026-09-03, commit `f24fe3a`), `horndb-bench materialize` on the taxonomy workload (depth 12, 40k instances; 40,012 asserted → **480,372 inferred**, 520,384 total, 4 rounds), median of 3 per backend. **Closure identical under both backends** (480,372 inferred). `closure_backend_ms`: rule-firing **7.99 ms** → GraphBLAS **2.97 ms** = **2.7× faster**. But end-to-end `reason_ms`: **519.7 → 518.8 ms = 0.2%**, i.e. inside the noise, because `closure_backend_ms` is only **~1.5% of `reason_ms`** — `compiled_rules_ms` (~234 ms) and `apply_ms` (~247 ms) are the real cost. | **AMBER — GraphBLAS wins its own phase and it does not matter here.** A 2.7× win on 1.5% of the work is a 0.2% end-to-end win. Backend selection is not a lever on this corpus shape; the levers are the compiled rules and the apply path. A corpus where transitive closure dominates (deep `owl:sameAs` / SKOS hierarchies) would weight it differently — **that corpus does not exist in-repo and was not invented for this run**. |
...
**The load legs predate HDB-158.** Every `read_turtle` / `read_ntriples` number
in this section — this table and the tuning passes below it — was measured with
a driver that parsed the whole corpus into one `Vec` of owned `oxrdf` terms
before interning. Since HDB-158 the driver flushes every 65,536 triples, the
same granularity `Store::load_*_file` inserts at. On trainmarks xlarge that is
**7.4% faster** (13.584s -> 12.573s on hornbench, one parse thread) and roughly
halves peak RSS, so the load rows here read a little slow. It also removed the
`--reserve-triples` flag, which existed only to preallocate that `Vec`; command
lines in the historical passes below still show it.
...
`dedupe_*` sub-counters (`HORNDB_DEDUPE_SUBPHASES=1`, removed along with the
`dedupe_*` phases themselves in HDB-104) on `hornbench`, trainmarks xlarge,
serial, commit `66e3302`; each column is the mean of two runs that agreed
within 2%. These runs predate the snmalloc swap (#293): the split is a share
of `dedupe`, so it holds, but the absolute seconds are glibc-allocator numbers
and the intern column — which allocates on every dictionary miss — is the one
most likely to move when they are re-measured.
...
- **`dedupe` drops 2.24s (ttl) / 2.16s (nt), well past the 1.4s HDB-90
  estimated.** Not an allocator effect: the isolated cost of 10M
  `HashSet<QuadKey>` inserts on an empty, warm set is ~0.7-0.75s on comparable
  hardware, consistent with HDB-90's 1.42s on a slower server core under
  glibc — nothing points to that isolated cost being bigger under snmalloc.
  The more likely mechanism is cache interference: `intra_batch.insert` and
  `Dictionary::intern_quad` ran interleaved in the same per-triple loop, and
  `intra_batch`'s ~550MB table being randomly accessed on every iteration
  evicted the dictionary's own hash table from L2/L3 and the TLB. HDB-90's
  split could not see that — the eviction cost landed in whichever of the two
  instrumented intervals ran next, mostly `dedupe_intern`'s, not
  `dedupe_intra`'s — so the isolated 1.4s figure structurally undercounts what
  removing `intra_batch` was actually worth. HDB-89 corroborates the
  contrast: it removed `live_keys`, a comparably-sized `HashSet`, but one
  built in a *separate* loop from the dictionary work, and that removal's
  `dedupe` stayed flat (5.255s -> 5.207s ttl) while `live_keys`'s own phase
  disappeared almost exactly its own size (1.291s) — no interleaving, no
  undercounting. This run, on the allocator actually in production, is the
  number to trust over the older instrumented split.
...
- **No other phase regressed.** `group`'s +0.01s (both legs) is inside the ~1%
  spread every other unmoved phase in this table shows. `merge` and `build`
  move too (`merge`'s ttl leg falls 42% but its nt leg only 3%), but both
  sit at the noise floor of a 13-15s run — 0.10-0.17s absolute for `merge` —
  so that inconsistency across legs is read as noise, not a mechanism worth a
  story.
...
| `benches/cold_tier.rs` | `horndb-storage` | SPEC-25 S5 cold-tier footprint and read amplification (HDB-181). Not a scaffold — measured on hornbench, see the `cold_tier` row in *Measured* above. Driven by `scripts/bench/cold-tier.sh` through `bench.yml`, which is the only route to hornbench from an agent session. Uses real LUBM if `LUBM_NT` names an existing file (the script fetches it via `get_lubm.sh`), else a synthetic LUBM-shaped generator; the summary says which. |
...
#### Scale: true SF=0.256 with editorial agents (HDB-37)
...
workload: the real SF=0.256 corpus with 2 editorial agents alongside the 4
aggregation ones, and **`editorial-qps` is the headline metric**.

**Series break.** `aggregation-qps` history does *not* span this change. The
corpus is ~500× bigger and the aggregation agents now share the store with
writers, so points before and after HDB-37 are not comparable. Both metrics
keep being recorded; read the older `aggregation-qps` points as a separate
series that ends on 2026-09-05.

Corpus build, hornbench (Ryzen 7 7700, 16 threads, 124 GiB, Debian 6.12,
2026-09-05, commit `f3b9d69`), via `scripts/bench/spb-sf256-build.sh`:
...
- **Materialization had to be sliced.** A single-process materialize costs
  ~3.8 kB per *asserted* triple (measured at 1/32 scale), which projects to
  ~900 GiB for 256 M — far past the host's 124 GiB, with a ceiling around
  29.7 M asserted triples. Slicing is sound because the closure of a subset is
  contained in the closure of the whole: at the 8 M calibration scale the
  sliced union lost **0** inferred triples and gained **0** extra, once
  blank-node lines are excluded (each materialize run mints its own `_:bN`
  labels, so those lines can never match textually across runs). Slices
  overlap on the shared ontology closure, so the concatenated file carries
  duplicate lines; the store folds them away on load, which is the gap between
  531.7 M lines and 465.6 M distinct triples.
- **Serving fits comfortably.** 115 B/triple here against the 170 B/triple
  HDB-146 measured on trainmarks — the SPB closure is dominated by a few
  repeated predicates, so the dictionary amortizes better.

Consequences for the scenario (`crates/harness/scenarios/spb-nightly.properties`):
a store-wide `SELECT (COUNT(*) …)` exceeds 300 s on this corpus while a typed
`GROUP BY` aggregation returns in 7.5 s, so `queryTimeoutSeconds` is raised
from 60 to 300 (a timeout is scored as an error) and `warmupPeriodSeconds`
from 5 to 60. The nightly also now waits on `/readyz` rather than `/query`:
...
# SPEC-25 S5 — cold-tier footprint + read amplification. Local run is a smoke
# check only; recorded numbers come from hornbench via bench.yml:
#   gh workflow run bench.yml --ref <branch> -f script=scripts/bench/cold-tier.sh
cargo bench -p horndb-storage --bench cold_tier
