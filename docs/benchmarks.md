...
| Steady-state memory, all views clean | one spine template, no per-view engine retained | hornbench (Ryzen 7 7700, 16 threads, Debian 6.12, rustc 1.90.0, 2026-09-03, commit `f24fe3a`): **RSS 24.9 MiB with all 1,000 views clean = 0.025 MiB (25.5 KiB) per view.** A retained per-view `Engine` would cost orders of magnitude more, so this confirms the design claim — only the spine template is held. **GREEN** |
