...
| owlrl semi-naïve firing A/B (`horndb-bench materialize --firing naive\|semi-naive`; compiled rules fire one delta-bound variant per body atom, SPEC-15 fix #2, HDB-40) | `horndb-owlrl` | cross-round re-derivation gone; closure identical; record `reason_ms` + rounds ([#134](https://github.com/sunstoneinstitute/horndb/issues/134)) | hornbench (16-core Ryzen, Linux 6.12.73, rustc 1.90.0, 2026-09-03, commit `f32420f`, `scripts/bench/seminaive-ab.sh`, same binary both arms), taxonomy d=12 / 40 k inst (480,372 inferred, 520,384 total), median of 3 per cell. Rule-firing backend: `compiled_rules_ms` naive **212.0 → semi-naïve 208.3 ms**, `reason_ms` **506.0 → 521.6 ms**, `apply_ms` 253 → 245. GraphBLAS backend: `compiled_rules_ms` 218.0 → 216.6 ms, `reason_ms` 503.5 → 518.0 ms. **Rounds 4 → 4, inferred identical (480,372), dumped closures identical (520,384 triples).** Within noise on this corpus: `cax-sco`'s round-4 work is dominated by *intra-delta* redundancy (the 68-pair `scm-sco` closure derives the same `x rdf:type Cj` through many paths, filtered by `contains`), which semi-naïve cannot remove; the old×old joins it does remove are a small share here, and building the per-round delta `MemStore` costs ~15 ms. **LUBM-1 leg — attempted, and blocked on a corpus-independent blowup, not on tooling.** The corpus is no longer the obstacle: LUBM-1/LUBM-10 now come from the `bench-corpora` release via `scripts/bench/get_lubm.sh`, and hornbench fetched both (run 33810258305). The A/B itself is blocked. **LUBM-1's OWL 2 RL closure under HornDB is 87,027,924 inferred triples from 100,866 asserted** — and **86,964,950 of them (99.93 %) have a *literal* subject**, so they are not even representable in RDF (`write_ntriples` skips them). That is exactly `L × (L + 1)` for the `L = 9,325` comparable literal terms in the corpus: `inject_datatype_literal_axioms` (`crates/owlrl/src/integration.rs`) asserts a symmetric `owl:differentFrom` (`dt-diff`) or `owl:sameAs` (`dt-eq`) for **every pair** of comparable literals before reasoning starts, so asserted size is quadratic in distinct literals on any literal-rich corpus. The non-literal-subject part of the closure is ~164 k triples, which matches the published LUBM-1 OWL 2 RL closure size; the other 87 M are pure injection. Consequences: one run costs 10.2 GiB peak RSS and ~38 s (so LUBM-10, ~13× the ABox, needs >100 GiB — it is not runnable), the run OOM-killed the bench host, and `compiled_rules_ms` is only **~4 % of `reason_ms`** on LUBM-1, so this corpus cannot discriminate the two firing strategies anyway. **Pre-existing, not from this branch**: `main`'s own `horndb-bench materialize` returns the identical 87,027,924 / 10.2 GiB on the same corpus, and this branch does not touch `inject_datatype_literal_axioms`. Tracked as `#TODO`; the semi-naïve A/B on a literal-rich corpus is blocked behind it. | **AMBER — parity GREEN, wall-clock flat on the taxonomy corpus, and the LUBM leg is blocked on a pre-existing quadratic literal-axiom injection (`#TODO`), not on this change.** The claim "semi-naïve removes cross-round re-derivation" is proven by construction and by `tests/semi_naive_differential.rs` (closure **and** round count identical to the naïve oracle, five corpora + proptest + both closure backends); the claim "and that makes materialization faster" is **not** demonstrated on any corpus measured so far ([#134](https://github.com/sunstoneinstitute/horndb/issues/134)) |
