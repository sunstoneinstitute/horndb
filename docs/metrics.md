...
| `tier` | `dram`, `cold`, `hbm`, `cxl`, `unknown` | `storage_tier_bytes_estimated` — `dram` (warm partitions' estimated DRAM) and `cold` (mmapped cold-partition file bytes) emitted; `hbm`/`cxl` reserved for SPEC-09; `unknown` no longer emitted |
...
| `horndb_storage_wal_appends_total` | counter | — | count | write batches appended to the write-ahead log (SPEC-25 S3). Zero for a store with no data directory, which logs nothing |
| `horndb_storage_wal_append_seconds` | histogram | — | s `(1e-5 ×4 ×12)` | wall-clock of one log append: encode, write, and whatever flush the store's `SyncPolicy` does. Under the default `EveryBatch` policy the flush is inside this window and happens before the write is acknowledged, so this histogram **is** the durability cost of a write — a p99 here is the first place to look when writes stall on a slow disk |
| `horndb_storage_wal_replay_records_total` | counter | — | count | log records replayed at startup (HDB-233). Non-zero only when the last run did not checkpoint before exiting; it is what a crash left to redo |
| `horndb_storage_wal_replay_seconds` | histogram | — | s `(1e-3 ×4 ×10)` | wall-clock of one whole startup replay, observed once per store opened. This is the startup cost of reopening a durable store, and what a tighter checkpoint cadence (`[server].checkpoint_interval` / `checkpoint_changes`) buys down |
