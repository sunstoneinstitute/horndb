...
| `chunk_pull` | `crates/sparql/src/exec/op/mod.rs` (`ChunkedBatch::next_chunk`) | handing a materialized `Batch` out one `batch_rows()`-sized chunk at a time: the `collect` that moves the rows plus the per-chunk `schema.clone()`. One clock per chunk (HDB-109) |
