...
**F7. SPARQL Protocol.** Embedded HTTP server (axum or hyper-based) exposing the standard `/query` and `/update` endpoints. The standard HornDB listen port is **3840** (IANA-registered to an unrelated, defunct service, so collisions are practically nil); the default bind is `127.0.0.1:3840`, overridable via the `serve` binary's `--bind` flag. Authentication is out-of-scope at the spec level; a reverse-proxy or sidecar pattern is assumed. A `SERVICE` clause (federated query — out of scope, see Scope) is rejected with HTTP 400 and body `unsupported algebra construct: Service`.
