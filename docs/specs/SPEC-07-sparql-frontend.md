...
**F7. SPARQL Protocol.** Embedded HTTP server (axum or hyper-based) exposing the standard `/query` and `/update` endpoints. The standard HornDB listen port is **3840** (IANA-registered to an unrelated, defunct service, so collisions are practically nil); the default bind is `127.0.0.1:3840`, overridable via the `serve` binary's `--bind` flag. Authentication is out-of-scope at the spec level; a reverse-proxy or sidecar pattern is assumed. A `SERVICE` clause (federated query — out of scope, see Scope) is rejected with HTTP 400 and body `unsupported algebra construct: Service`. The server also exposes `GET /healthz` (process up — always `200`) and `GET /readyz` (`200` once the startup data load, and any `--materialize` pass, finishes; `503` before that) for Kubernetes liveness/readiness probes; every response carries an `x-request-id` header (passed through or generated) that also appears in the access log, so a slow or failing request can be traced back to its client; and `serve` stops accepting new connections on SIGTERM/SIGINT, giving in-flight requests up to a configurable drain timeout (`[server].shutdown_drain`, SPEC-26) to finish before it force-exits.
