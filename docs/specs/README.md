...
| [SPEC-26](SPEC-26-config-system.md) | **Configuration system** — layered TOML config (base `config.toml` + `config.d/` drop-ins, merged by documented precedence), live watch/reload via `notify` + `ArcSwap`, and a two-tier server-vs-query settings model with per-query overrides (URL params + a ClickHouse-style `SETTINGS` clause); wires `bind`, query timeout, result-row cap, and `rdf12` to real config. New `horndb-config` crate. Per-session memory accounting delegated to a companion spec | SPEC-07 (server surface) + SPEC-17 (reload metrics) | Draft |
