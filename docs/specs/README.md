...
| [SPEC-28](SPEC-28-named-graph-dataset-semantics.md) | **Named-graph and dataset semantics** — `GRAPH` in query and update, `FROM`/`FROM NAMED` dataset construction, named-graph graph-management verbs, the Graph Store Protocol, the graph-scoped access paths those need from storage, and idempotent quad-grain apply. Storage has been quad-aware since SPEC-25 S1; phases 1–3 have brought query up to it — `GRAPH`, `FROM`/`FROM NAMED` and the `union`/`strict` default-graph mode evaluate, bar two families of `GRAPH ?g` query that are refused rather than answered wrongly. Update, GSP and idempotent apply (phases 4–5) are still open | SPEC-07 (+ SPEC-02 F7 access paths); epic [#189](https://github.com/sunstoneinstitute/horndb/issues/189), supersedes [#54](https://github.com/sunstoneinstitute/horndb/issues/54) | Draft |
