...
| [SPEC-26](SPEC-26-config-system.md) | **Configuration system** — layered config (built-in defaults < argv < env < base `config.toml` < `config.d/*.toml` drop-ins, merged by documented precedence), live watch/reload via `notify` + `ArcSwap`, and a two-tier server-vs-query settings model with per-query overrides via URL query parameters; wires `bind`, the SIMD knobs, query timeout, result-row cap, and `rdf12` to real config. New `horndb-config` crate. Per-query memory accounting delegated to a companion spec | SPEC-07 (server surface) + SPEC-17 (reload metrics) | Draft |
