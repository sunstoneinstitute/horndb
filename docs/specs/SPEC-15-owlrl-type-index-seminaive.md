...
| 4 — compiled-rule + apply share of reason time must fall | **NOT met (and not yet settled)** | On the taxonomy corpus the share is flat: `compiled_rules_ms` 212.0 → 208.3 ms inside a `reason_ms` that did not move (hornbench, run 33807781901). The LUBM-1 A/B that was supposed to settle it was blocked by the all-pairs `dt-eq`/`dt-diff` literal injection (87 M junk triples, OOM-killed the bench host) — **fixed by HDB-147** (O(k) bucketed `dt-eq`, `dt-diff` derived lazily from `owl:sameAs` pairs). The LUBM-1/LUBM-10 re-run on hornbench is **pending**: the runner was offline when HDB-147 landed. See `docs/benchmarks.md`. |
