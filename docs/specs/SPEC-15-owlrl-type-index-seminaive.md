...
| 4 — compiled-rule + apply share of reason time must fall | **NOT met (and not yet settled)** | On the taxonomy corpus the share is flat: `compiled_rules_ms` 212.0 → 208.3 ms inside a `reason_ms` that did not move (hornbench, run 33807781901). The LUBM-1 A/B that was supposed to settle it is blocked — see `docs/benchmarks.md`: LUBM-1's closure explodes to 87 M triples through a pre-existing all-pairs literal axiom injection, compiled-rule firing is only ~4 % of `reason_ms` there, and the run OOM-killed the bench host. Follow-up: `#TODO`. |
