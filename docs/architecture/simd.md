...
|---|---|---|
| `wcoj/src/executor/wcoj.rs:524`, `trie/leapfrog.rs:131` | `intersect` | the `k == 2` leapfrog fast path: intersect two `active_run` slices in one bulk call instead of round-robin seeking |
| `wcoj/src/source/vec_source.rs:285` (`VecIter::seek`) | `lower_bound` | far-target seek within a stored level column |
