...
| `rdf:type` skew parallelism (F5) | **partially implemented (list-rule path + compiled-rule object index landed; semi-naïve firing planned)** | The `rdf:type`-driven hand-written list rules (`cls-int1`, `cls-uni`, `cax-adc`, `prp-key`) partition their per-subject filtering by class id and parallelise it across rayon above `PAR_TYPE_THRESHOLD` (`crates/owlrl/src/list_rules.rs`), selected by `MaterializeOpts::parallel` (`ParallelStrategy::Auto` default; `Serial` is the oracle). Identical closure proven by `tests/rdf_type_skew_differential.rs` (3 large-extent fixtures + proptest); `benches/rdf_type_skew.rs` + `docs/benchmarks.md` record the win ([#39](https://github.com/sunstoneinstitute/horndb/issues/39)). The **compiled** (`cax-sco`-style) rules are the open hotspot, but profiling ([#133](https://github.com/sunstoneinstitute/horndb/issues/133)) shows the cost is an un-indexed full `rdf:type`-partition scan + naïve (non-delta) re-firing, **not** a parallelism gap. Fix #1 — a within-partition object index on `MemStore` (`obj_index`: predicate → object → subjects; no `FireFn`/trait change) — **landed** ([#133](https://github.com/sunstoneinstitute/horndb/issues/133), 2026-07-07): `probe(None, p, Some(o))` is now O(\|extent\|), cutting `compiled_rules_ms` ~17% on the LUBM-shaped A/B (hornbench; closure bit-identical) — see `docs/benchmarks.md`. Fix #2 — genuine delta-driven semi-naïve firing (which does change the `FireFn` signature, removing the ~4× cross-round re-derivation) — is still **planned** ([#134](https://github.com/sunstoneinstitute/horndb/issues/134)). Both specified in `docs/specs/SPEC-15-owlrl-type-index-seminaive.md`. |
...
The rest of the Stage-2 completeness work —
change-feed net-delta + backpressure, engine wiring (the crate has no
consumers yet), DeltaLog WAL, MVCC backing of snapshots, and the
bilinear-join runtime — remains **planned** under `SPEC-24` (epic
[#186](https://github.com/sunstoneinstitute/horndb/issues/186)), decomposed
into `TASKS.md` phase tasks
[#212](https://github.com/sunstoneinstitute/horndb/issues/212)–[#217](https://github.com/sunstoneinstitute/horndb/issues/217).
...
| Pattern-based Update (`INSERT`/`DELETE … WHERE`, `DELETE WHERE`, `WITH`/`USING`/`USING NAMED`, multi-op) | **implemented** | `update.rs::apply_delete_insert`: evaluates the WHERE pattern via `translate_where` → planner → runtime, collects all solutions over the pre-update graph, then applies deletions-before-insertions (SPARQL 1.1 §3.1.3) as one `apply_quads` batch. Named-graph templates resolve each template quad's graph (named / default / a row-bound variable); `WITH`/`USING`/`USING NAMED` build the WHERE dataset through the SPEC-28 phase-3 `DatasetSpec` machinery. Multi-operation (`;`-joined) sequences each apply as their own batch/commit. Ground-template safety drops triples with unbound slots; per-solution blank nodes are row-scoped. **Known gap:** a bare `WITH <g>` (no `USING`) wrongly empties named-graph visibility for a ground `GRAPH` block in WHERE ([#281](https://github.com/sunstoneinstitute/horndb/issues/281); 2 W3C cases excluded, see `harness/KNOWN-MANIFEST-BUGS.md`). ([#51](https://github.com/sunstoneinstitute/horndb/issues/51), [#267](https://github.com/sunstoneinstitute/horndb/issues/267), `PLAN-28-04`) |
...
| RDF 1.2 triple-term patterns `<<( s p o )>>` | **implemented (gated)** | Accepted only when the caller's `SparqlConfig.rdf12` is `true`: library callers pass `SparqlConfig::rdf12()` directly; over HTTP it now (`PLAN-28-03` Task 2) comes from `[server.limits].rdf12` — `AppState.cfg`, built once from the loaded config in `serve.rs`, replaced the previous hardcoded `SparqlConfig::default()` on both query handlers. Default (`rdf12 = false`) keeps SPARQL 1.1 callers on 1.1 semantics. `translate_query_with` / `execute_query_with`. |
...
| Graph-management Update (`LOAD`/`CLEAR`/`DROP`/`CREATE`/`ADD`/`MOVE`/`COPY`) + multi-op updates | **implemented** | `update.rs` follows D11 existence semantics: a graph exists iff it holds ≥1 visible quad — no registry. `CREATE <g>`: no-op if absent, error if present unless `SILENT`. `CLEAR`/`DROP <g>`: error if absent unless `SILENT`; present → every visible quad retracted through `apply_quads` (never a structural unlink, so the delta/change-feed path sees it). `CLEAR DEFAULT` sweeps only the default-graph sentinel, split from `CLEAR ALL` (every non-reserved graph plus default); `DROP ALL` likewise spares the reserved namespace. `LOAD <src> [INTO GRAPH <g>]`: triples formats route to the destination (default graph if no `INTO`); dataset formats (`.nq`/`.trig`) route each quad to its own named graph on a plain `LOAD`, and `LOAD … INTO GRAPH` of a dataset format is an error (redirecting quads is undefined by W3C LOAD); remote (`http(s):`) sources stay unsupported (`file:`-only, → E5 [#189](https://github.com/sunstoneinstitute/horndb/issues/189)). `ADD`/`MOVE`/`COPY` arrive spargebra-desugared into `Drop`+`DeleteInsert` pairs, which now execute against real named graphs; **`SILENT` fidelity** is recovered with a source-text tokenizer that records `(verb, silent)` per occurrence and aligns it with the desugared pairs by order, because spargebra 0.4.6 drops the flag on desugaring (upstream issue not yet filed, `#TODO` — this machinery is deletable once it ships). **The reserved namespace** (`https://horndb.io/graph/`) is closed to every write form, including a runtime-bound `GRAPH ?g` template, and is **not** suppressible by `SILENT`. **Atomicity:** a `validate_op` preflight mirrors every apply-time check over the whole multi-op sequence before any mutation runs; because the preflight checks the *initial* store state, one request that both creates and uses a graph (e.g. `INSERT DATA {GRAPH <g>{…}}; DROP <g>`) false-errors — conservative but wrong, a known gap (`#TODO`, not yet filed). Conformance: W3C `add/`, `copy/`, `move/`, `clear/`, `drop/`, `delete/` families, 45 of 47 cases selected and green on both backends (`harness/selected.toml` `[sparql_update]`); the rest in `harness/KNOWN-MANIFEST-BUGS.md`. Tests: `tests/update_named_graph.rs`, `tests/update_graph_mgmt.rs`, `tests/update_feed_replay.rs`, `tests/w3c_update_suite.rs` (both backends), `tests/server_http.rs`. ([#52](https://github.com/sunstoneinstitute/horndb/issues/52), [#267](https://github.com/sunstoneinstitute/horndb/issues/267), `PLAN-28-04`) |
...
| Graph Store Protocol | **specified** ([#268](https://github.com/sunstoneinstitute/horndb/issues/268); plan at pickup) | `SPEC-28` phase 5. Direct REST access to *named* graphs. The earlier "blocked on storage" note was **wrong and is retired**: storage has been quad-aware since SPEC-25 S1 ([#225](https://github.com/sunstoneinstitute/horndb/issues/225)) — `Store::insert_quads`/`retract_quads`/`intern_graph_uri` all take a `GraphId` and `MemoryTier` keys partitions by graph. The gap is entirely *above* storage (see the two rows below). GSP is standalone-product surface, not on the data-platform integration path. ([#54](https://github.com/sunstoneinstitute/horndb/issues/54), closed — superseded by `SPEC-28`) |
| Named-graph query + update scoping (`GRAPH`, `FROM`/`FROM NAMED`) | **implemented (phases 1–4); GSP (phase 5) planned** ([#267](https://github.com/sunstoneinstitute/horndb/issues/267), `PLAN-28-04`) | `SPEC-28` phases 1–4, all done. Phase 1 ([#264](https://github.com/sunstoneinstitute/horndb/issues/264)) refused `GRAPH` and the dataset clause instead of answering them wrongly; phase 2 ([#265](https://github.com/sunstoneinstitute/horndb/issues/265)) supplied the storage plumbing (`graph_len`, graph-scoped scan/iteration — see the SPEC-02 section above); phase 3 ([#266](https://github.com/sunstoneinstitute/horndb/issues/266), `PLAN-28-03`) replaced the refusal with real query evaluation — `Algebra::Graph`, scan-scope lowering, `GRAPH ?g` as a scan column, `DatasetSpec`, the `union`/`strict` default-graph mode, scope-aware pushdowns, path scoping, and the W3C `graph/` + `dataset/` families (only the two `GRAPH ?g` shapes in the `GRAPH` row above still 400). Phase 4 ([#267](https://github.com/sunstoneinstitute/horndb/issues/267), `PLAN-28-04`) closes the write side: `INSERT`/`DELETE DATA` and pattern templates target real named graphs, graph management (`CREATE`/`CLEAR`/`DROP`/`LOAD`/`ADD`/`MOVE`/`COPY`) follows D11 existence semantics, and the store boundary (`Store::apply_quads`) is atomic and idempotent per batch — see the SPARQL Update rows and the SPEC-02 section above. Known gaps: [#281](https://github.com/sunstoneinstitute/horndb/issues/281) (bare `WITH` visibility) and a create-then-use preflight false-error (`#TODO`, not yet filed) — both detailed on the Update rows below. Phase 5 (Graph Store Protocol, row below) is still planned. |
| Named-graph reasoning scope | **planned (P1)** ([#269](https://github.com/sunstoneinstitute/horndb/issues/269), `PLAN-29-01`; P2–P4 specified) | `SPEC-29`. What OWL 2 RL reasons over in a many-graph store, and where derived triples land. `Engine::load` (`owlrl/src/integration.rs`) currently skips every non-default-graph quad, so a store of named graphs reasons to an empty closure and reports no error. Derived triples go to per-view inferred graphs under a reserved namespace, never into the source graph — a source-graph read must stay asserted-only or a whole-graph `PUT` diff deletes inferences the client never sent. |
| Change-feed materializer (apply, cursor, recovery) | **planned (P1)** ([#270](https://github.com/sunstoneinstitute/horndb/issues/270), `PLAN-30-01`; P2–P4 specified) | `SPEC-30`. The durability contract HornDB offers a consumer that applies an external `{adds, dels}` feed and holds its own cursor: applied-state guarantees across restart, startup reconciliation against that cursor, and rebuild-from-zero. Exists because SPEC-02 NF5 accepts losing updates between checkpoints (WAL is `SPEC-25` S3, [#227](https://github.com/sunstoneinstitute/horndb/issues/227), planned), so a naive apply-then-advance consumer can diverge from HornDB permanently and silently. |
| Streaming result serialization (F6) | **implemented** | Both the per-node buffering (#143 streaming runtime) and the whole-body buffering are gone. `Runtime::run_stream` yields a `BindingsStream`; `server/query.rs::stream_select` pulls it chunk-by-chunk through per-format incremental serializers into a chunked `ChannelBody`, with a sized-body fast path when a SELECT fits one chunk. All four SELECT formats (JSON/XML/CSV/TSV) stream. Design: `docs/specs/SPEC-22-http-streaming-results.md` + `docs/plans/PLAN-22-01-http-streaming-results.md`. ([#56](https://github.com/sunstoneinstitute/horndb/issues/56), closed; delivered under [#128](https://github.com/sunstoneinstitute/horndb/issues/128)) |
| SPARQL 1.1 Federation (`SERVICE`) | **deferred** | Indefinitely. |
| Operator configuration system | **partially implemented** (see §15) | `SPEC-26` (approved) — layered config (built-in defaults < base `config.toml` < `config.d/*.toml` drop-ins < env < argv), live watch/reload (`notify` + `ArcSwap`), and a two-tier server-vs-query settings model with per-query URL-param overrides; wires `bind`, the `[simd]` knobs, `query_timeout`, `max_result_rows`, and `rdf12` to real config (`max_query_memory` parsed but enforcement delegated to a companion spec). New foundation crate `horndb-config`. Four leaf tasks: Phase 1a `horndb-config` crate ([#249](https://github.com/sunstoneinstitute/horndb/issues/249), `PLAN-26-01`), Phase 1b serve wiring + `[simd]` injection ([#250](https://github.com/sunstoneinstitute/horndb/issues/250), `PLAN-26-02`), Phase 2 query overrides + enforcement ([#251](https://github.com/sunstoneinstitute/horndb/issues/251)), Phase 3 live watch/reload ([#252](https://github.com/sunstoneinstitute/horndb/issues/252)). Phases 1a/1b have landed; Phase 2's general override mechanism has not, though `default_graph` is already live per query via SPEC-28 phase 3. |
...
| `Dataset` / `ConjunctiveGraph` named-graph facades | **planned** | F3; the Python facade itself is unbuilt. The underlying store gained named-graph query and update support in `SPEC-28` phases 1–4 ([#267](https://github.com/sunstoneinstitute/horndb/issues/267)); no Python surface exposes it yet. |
...
**Crate:** new `horndb-config` (dependency-light leaf) · **Spec:** `SPEC-26` · **Overall status: partially implemented** (library and `serve` wiring landed — Phases 1a/1b; live watch/reload and the general per-query URL-override mechanism remain planned, though one key — `default_graph` — is already live via SPEC-28 phase 3)
...
| Live watch/reload, per-query URL overrides + enforcement | **planned** | SPEC-26 S3/S4/S5 (later phases, [#251](https://github.com/sunstoneinstitute/horndb/issues/251)/[#252](https://github.com/sunstoneinstitute/horndb/issues/252)). One key of S4's whitelist, `default_graph`, is threaded early by SPEC-28 S3 (`PLAN-28-03` Task 2, [#266](https://github.com/sunstoneinstitute/horndb/issues/266)): `AppState.cfg` and a validated `default_graph` URL/form override exist on `/query` today (config-key spelling, not `default-graph` — kept clear of the SPARQL 1.1 Protocol's reserved `default-graph-uri`, which SPEC-28 phase 5's GSP needs on the same endpoint); the other four `QuerySettings` keys, and the general whitelist mechanism, remain planned. |
...
question is resolved **provisionally** as recompute-from-snapshot (no
incremental maintenance under SPEC-06 deltas yet; see `PLAN-23-03`). Later
phases ([#204](https://github.com/sunstoneinstitute/horndb/issues/204)–[#207](https://github.com/sunstoneinstitute/horndb/issues/207))
