...
| Cardinality estimation | **implemented** | `cardinality.rs`, `estimator.rs`, `stats.rs`. `StatsEstimator` over a recompute-from-snapshot `SnapshotStats`; reached from SPARQL through `HornBackend::cardinality_estimate`, which today serves `EXPLAIN` only. |
| Cost-based plan choice (estimator drives the plan) | **planned** | **The estimator is built but not connected to planning.** `Planner::choose` (`planner.rs`) takes a `Cardinality` as `_est` and discards it; `ExecutionPlan::for_bgp` (`plan.rs`) is never handed one. Both decisions it makes are structural: WCOJ vs binary-hash on a fixed 4-pattern cutover, and the WCOJ variable elimination order by **descending pattern degree** (how many patterns mention a variable), with no selectivity input. HDB-108 measured what that costs on trainmarks q3 at xlarge: the degree heuristic puts `?order` (degree 4) at depth 0 ahead of `?customer` (degree 3), so the join enumerates all 1,335,000 orders before applying the selective `:country :Norway` and discarding 90% of them — 10.1M leapfrog seeks and 95% of warm exec time in `scan_wcoj`. The same query planned `?customer`-first runs at 0.20-0.33s warm against 0.83s (0.33s for the strictly comparable variant, same rows and columns). Numbers and the variant-query evidence: `docs/benchmarks.md`. This is **SPEC-03 F6** ("planner uses [per-predicate statistics] for join-order selection and WCOJ-vs-binary-join cutover") unmet — not F2, which is only the cutover. **Already tracked: HDB-46** (SPEC-23 Phase 4, cost-based `JoinPlanning`), plan `docs/plans/PLAN-23-04-cost-based-join-planning.md`, which names this defect at lines 33-34 and whose Task 5 ("Greedy WCOJ variable ordering… seeded by descending degree") is the fix. HDB-108 is supporting evidence for prioritizing it: on q3 the variable ordering alone accounts for the whole gap, with no contribution from the structural routing or the WCOJ-vs-hash choice. PLAN-23-04 is still `BLOCKED` on five of its six listed prerequisites (the sixth, PLAN-23-03's statistics seam, is executed and its checkbox merely stale), and Task 5 depends on the per-subplan plan IR and `JoinSpec` that Tasks 1-4 build, so it is not independently pullable as written — see `docs/benchmarks.md` for what a narrower interim slice would and would not cover. Do not file a new task for this. |
