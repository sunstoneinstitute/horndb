...
| Bilinear rule operator (two-pattern bodies) | **implemented** | `operator.rs`, `circuit.rs`. `NaryPlan` binds each join leaf to a per-predicate slice of the base extent (`PredExtent`, `SPEC-24` S7) instead of scanning the whole extent on every join — a rule's `body_predicates()` declares which predicate each leaf reads, `None` for a leaf with a variable in predicate position (e.g. prp-dom). Cost-based leaf ordering and hash/sort-merge join kernels remain **planned** (`SPEC-24` S7, [#216](https://github.com/sunstoneinstitute/horndb/issues/216)). |
