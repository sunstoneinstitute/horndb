...
extents and the `HashJoinRule` kernel have landed (see the bilinear rule
operator row below); cost-based leaf ordering, a sort-merge kernel, and
codegen emitting kernel instances are still planned. The intra-tick joint
