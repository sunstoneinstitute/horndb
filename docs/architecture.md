...
| LDBC SPB nightly throughput A/B (`.github/workflows/nightly.yml`) | **implemented (feasible scale)** | Per-run HornDB bring-up via `crates/harness/scripts/start-engine.sh` (serving the prepared flat closure, no reasoning); `harness spb-run` drives the SPB aggregation mix against `/query`+`/update` and records `aggregation-qps` to the trend DB. A/B reference is **GraphDB Free 10.8.14** (licence-free; 11.x hard-requires a licence), a standing service on `hornbench` provisioned by `scripts/bootstrap-graphdb-free-spb.sh`; the leg skips gracefully if GraphDB is down. Validated 2026-06-25 at *feasible scale* (512 k-triple SPB closure, aggregation-only). **Follow-up:** true SF=0.256 (256 M triples) + editorial agents, and GraphDB systemd durability (TASKS.md). See `BENCHMARKS.md`. |
...
| trainmarks (DataTreehouse) end-to-end benchmark | **runnable** | All six trainmarks queries execute on `HornBackend` at all three scales (100K/1M/10M). Native driver `crates/bench-trainmarks` + `scripts/bench/trainmarks.sh`; numbers in `BENCHMARKS.md`. Known cliff: q4's `OPTIONAL` left-join is ~quadratic (nested-loop) and times out at scale — the headline follow-up. |
