...
| WCOJ seek + leapfrog intersect SIMD | **implemented** | F1; highest payoff. Seek: `VecIter` builds a transient SoA `LevelColumn` for the **depth-0 (root, full-data) level** — built once, reused for the whole scan — and seeks it through `horndb_simd::lower_bound`; deeper levels stay on the scalar AoS `partition_point` to avoid an O(range) per-`open_level` column rebuild in the inner loop (a single bound predicate makes the depth-1 child range nearly the whole dataset). `PackedColumn::lower_bound` (compressed source) bisects to the owning block, decodes it, and SIMD-finishes (`source/soa.rs`, `source/vec_source.rs`, `source/packed_column.rs`). Intersect: `LeapfrogJoin` gains a k==2 fast path over `active_run` contiguous views via `horndb_simd::intersect` (`trie/leapfrog.rs`). Output bit-identical to scalar — gated by the WCOJ differential fuzzer (seek path) + the leapfrog BTreeSet oracle (intersect). **Note:** the production executor (`executor/wcoj.rs::BatchIter`) runs its own inlined leapfrog (not `LeapfrogJoin`), so the SIMD *seek* path is live in production while the `LeapfrogJoin` intersect fast path currently benefits only the `trie/leapfrog.rs` consumers; wiring it into `BatchIter` is a follow-up (tracked in `TASKS.md`). `per_tuple` ns/tuple pending hornbench. |
