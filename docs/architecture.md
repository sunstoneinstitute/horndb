...
| LDBC SPB nightly throughput A/B (`.github/workflows/nightly.yml`) | **implemented (feasible scale)** | Per-run HornDB bring-up via `crates/harness/scripts/start-engine.sh` (serving the prepared flat closure, no reasoning); `harness spb-run` drives the SPB aggregation mix and records the full driver report to the trend DB. A/B reference is **GraphDB Free 10.8.14**, brought up per run so neither engine competes for RAM during the other's measurement; the leg skips gracefully if GraphDB fails to start. Runs at *feasible scale* (512 k-triple SPB closure, aggregation-only); true SF=0.256 + editorial agents is a TASKS.md follow-up. Numbers: `docs/benchmarks.md`. |
...
| Streaming result serialization (F6 — currently buffers) | **planned** | The per-node buffering is gone (#143 streaming runtime), but `Runtime::run` still collects a full `Vec<Bindings>` and the serializers buffer the whole body. Planned 2026-07-06: `docs/specs/2026-07-06-http-streaming-results-design.md` + `docs/plans/2026-07-06-http-streaming-results.md` — `Runtime::run_stream` + incremental serializers + chunked HTTP bodies for all four SELECT formats. ([#56](https://github.com/sunstoneinstitute/horndb/issues/56), closed; tracked under [#128](https://github.com/sunstoneinstitute/horndb/issues/128)) |
