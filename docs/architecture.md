...
| Runtime ISA dispatch (cached fn-ptr, `is_*_feature_detected!`, no nightly) | **implemented** | NF5; cached `OnceLock` fn-ptr per primitive, scalar-forced build green on stable 1.90. F5 `with_forced_isa` makes dispatch test-forceable so the differential suite exercises every host ISA path. Operational cap `HORNDB_SIMD_MAX_ISA` (`scalar`/`avx2`/`neon`/`avx512`, read once) clamps production selection without a rebuild — e.g. `=avx2` disables AVX-512 if Zen4 downclocking loses net, `=scalar` is a SIMD kill-switch; queryable via `configured_max_isa()`. Does not affect the test/bench force. **Startup micro-calibration** (`HORNDB_SIMD_AUTOTUNE`, default on; `=off` falls back to widest-available static preference) times each cap-allowed kernel per primitive on a synthetic L2-resident workload at first use / `horndb_simd::init()` and caches the fastest, only adopting a SIMD kernel when it beats scalar by ≥5%. This auto-handles host-dependent reversals — measured (2026-06-30): AVX-512 `intersect` wins ~2.5× on Intel Sapphire Rapids (hel01) but regresses ~2.5× on Zen4 (hornbench) double-pumped 512-bit units; `lower_bound` SIMD is an algorithmic loss on both → scalar — with **no hardcoded CPU-model blocklist**. The selected ISA per primitive is exported as the `horndb_simd_kernel_isa{kernel,isa}` gauge (`crates/metrics/src/simd.rs`), set once at server startup from `serve.rs` via `calibration_report()`. |
