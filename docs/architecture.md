...
| W3C OWL 2 RL test-suite ingestion pipeline | **implemented** | `owl2_rl_extract.rs` + `harness extract-owl2-rl`; 115 W3C cases → 97 green in `[suites.owl2-w3c-rl]`, 18 reds tracked in `harness/KNOWN-MANIFEST-BUGS.md` ([#160](https://github.com/sunstoneinstitute/horndb/issues/160) tracks the RL-reachable remainder). |
...
| Algebra translation (BGP, Join, LeftJoin, Filter, Project, Distinct, Slice, OrderBy, Union, Extend, Values) | **implemented** | `algebra/translate.rs`. All 13 runtime operators run native on id-carrying slot rows (`Slot`/`Row`/`Batch`) after Slice 2 of [#128](https://github.com/sunstoneinstitute/horndb/issues/128). `OPTIONAL`/`LeftJoin` executes as a slot hash-left-join keyed on the shared (join) variables — `exec/runtime.rs` `LeftJoin` arm + `probe_into_slots`, ~linear in the common case (was a quadratic nested loop; trainmarks q4 cliff, #116); the inner `Join` arm is **likewise a hash join** over the same `batch_join_vars`/`row_join_key` machinery — right relation indexed once, each left row probing its key bucket plus the conservative `unkeyed`/None fallbacks — replacing its earlier O(\|l\|·\|r\|) nested loop ([#128](https://github.com/sunstoneinstitute/horndb/issues/128) increment 1, #141); `merge_rows`/`merge_rows_with` apply the slot compatibility rule (with the per-join column-index lookups hoisted to a once-per-join `build_merge_plan`) and `normalize_columns` restores within-column homogeneity where a matched/unmatched provenance mix would otherwise split a downstream `DISTINCT`/`GROUP BY` key. |
| Aggregation / `GROUP BY` (`COUNT`/`SUM`/`MIN`/`MAX`/`AVG`/`SAMPLE`/`GROUP_CONCAT`, `DISTINCT` modifiers) | **implemented** | `algebra/translate.rs` + `exec/runtime.rs::eval_group_native`. Unblocks the LDBC SPB aggregation mix (incl. the driver's `COUNT` warm-up query). #66. **Perf ([#128](https://github.com/sunstoneinstitute/horndb/issues/128) — Slice 1 + Slice 2 landed):** `eval_group_native` keys groups on raw-id `KeyPart`s (no per-row `TermId → String` decode); `COUNT(*)` is `members.len()` (zero column access); value aggregates decode the **union** of all aggregates' referenced columns **once per group** via `decode_subset` and share it (no per-aggregate re-decode of the same column), and the per-group key-slot row is **moved, not cloned**, into the output ([#128](https://github.com/sunstoneinstitute/horndb/issues/128) increment 2, #167). `DISTINCT` dedup hashes on `Vec<KeyPart>`. **Official `aggregation-qps` (hornbench SPB-256 nightly, recorded):** HornDB **~13 → ~23 qps** post-Slice-1 (~1.77×) vs GraphDB Free ~148–154; the ~12× gap narrowed to **~6.5×**. Slice 2 ported the remaining six operators to native slots and removed the decode-adapter + `cfg(test)` legacy oracle. Streaming runtime (#143) and COUNT-over-BGP pushdown (#144) landed (2026-06-30). SPB-256 re-measured on hornbench 2026-06-30: **~30.8 qps vs GraphDB Free ~153 (~5.0× gap, was ~6.5×)**. The 23 → ~30.7 gain was bisected (locally) to Slice 2's native-slot `LeftJoin`/`OPTIONAL` hash probe (`309c2db`) — the SPB aggregation mix is `OPTIONAL`-heavy — *not* #143/#144 and *not* the metrics batch; #143/#144 are net-neutral on top (branch 30.78 ≈ same-day main 30.71). Deferred: probe-side join streaming (joins drain both children before output); filter-aware/multi-aggregate count pushdown; result streaming to the HTTP layer. |
...
| Runtime ISA dispatch (cached fn-ptr, `is_*_feature_detected!`, no nightly) | **implemented** | NF5; cached `OnceLock` fn-ptr per primitive, scalar-forced build green on stable 1.90. F5 `with_forced_isa` makes dispatch test-forceable so the differential suite exercises every host ISA path. Operational cap `HORNDB_SIMD_MAX_ISA` (`scalar`/`avx2`/`neon`/`avx512`, read once) clamps production selection without a rebuild — e.g. `=avx2` disables AVX-512 if Zen4 downclocking loses net, `=scalar` is a SIMD kill-switch; queryable via `configured_max_isa()`. Does not affect the test/bench force. **Kernel selection (reworked 2026-07-01)** resolves each primitive through the priority `forced → HORNDB_SIMD_MAX_ISA cap → known-CPU table → representative-input calibration → static widest`. A same-session LDBC SPB-256 A/B on Zen4 (hornbench) and Intel SPR (hel01) proved the previously-calibrated SIMD kernels **net-harmful vs scalar on both** (dominant culprit: AVX2 `lower_bound` on the seek-heavy leapfrog path; the earlier "AVX-512 `intersect` ~2.5× on Intel" was a *microbench-only* result contradicted by the real workload, where AVX-512 runs ~half scalar throughput). The **known-CPU table** (`cpu.rs`, CPUID vendor/family/model keyed, SPB-derived) pins scalar for both measured hosts (AMD fam 25 mdl 97 Ryzen 7 7700, Intel fam 6 mdl 143 Xeon Gold 5412U) with no timing; **representative-input calibration** (`HORNDB_SIMD_AUTOTUNE`, default on; `=off` → static widest) is the fallback for unlisted CPUs, timing each cap-allowed kernel on inputs shaped like production access (seek-sweep for `lower_bound`, >L2 base for `gather`, moderate selectivity for `filter_indices_eq`) and adopting SIMD only when it beats scalar by ≥5% — so an unknown host also rejects the killer kernels. SPB-256 aggregation-qps recovered on Zen4 (28.6 regression → **36.16**; Intel steady 34.4). The selected ISA and selection tier per primitive are exported as the `horndb_simd_kernel_isa{kernel,isa,source}` gauge (`crates/metrics/src/simd.rs`), set once at server startup from `serve.rs` via `calibration_report()`. |
