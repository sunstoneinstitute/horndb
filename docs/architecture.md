...
| Aggregation / `GROUP BY` (`COUNT`/`SUM`/`MIN`/`MAX`/`AVG`/`SAMPLE`/`GROUP_CONCAT`, `DISTINCT` modifiers) | **implemented (correct, slow)** | `algebra/translate.rs` + `exec/runtime.rs::eval_group`. Unblocks the LDBC SPB aggregation mix (incl. the driver's `COUNT` warm-up query). #66. **Perf:** ~12× behind GraphDB on the SPB aggregation mix — `eval_group` keys a `BTreeMap` on per-row re-`lex()`ed strings and `DISTINCT` dedup is a linear scan (O(n·d)). Structural fix (hash group-by, id-based bindings, streaming) tracked in [#128](https://github.com/sunstoneinstitute/horndb/issues/128); the O(n²) `DISTINCT` scan is fixed separately. |
