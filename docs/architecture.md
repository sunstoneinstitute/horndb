...
| LDBC SPB nightly throughput A/B (`.github/workflows/nightly.yml`) | **implemented (feasible scale); SF=0.256 cutover open in [#362](https://github.com/sunstoneinstitute/horndb/pull/362)** | Per-run HornDB bring-up via `crates/harness/scripts/start-engine.sh` (serving the prepared flat closure, no reasoning); `harness spb-run` drives the SPB aggregation mix and records the full driver report to the trend DB. A/B references are **GraphDB Free 10.8.14** and **Oxigraph 0.5.9** (the latter a Rust/RocksDB SPARQL store with no reasoner — the closest architectural peer, serving the same flat closure; run as two legs, `oxigraph` as-loaded and `oxigraph-optimized` from an `oxigraph optimize`d store copy), each brought up per run so no engine competes for RAM during another's measurement; each leg skips gracefully if that engine fails to start. The Oxigraph legs need a one-time persisted-store build on the runner (`bootstrap-oxigraph-spb.sh`, builds both stores) before they record. The trend DB keeps a 90-day rolling window (`harness prune`). Since HDB-37 (2026-09-05) it runs the LDBC-intended workload: the true SF=0.256 corpus — 256,000,196 asserted Creative Work triples whose OWL 2 RL closure is **465,633,142 distinct triples** — with 2 editorial (Creative Work insert/update/delete) agents alongside the 4 aggregation ones, and the headline metric moved from `aggregation-qps` to `editorial-qps`. **The `aggregation-qps` series breaks there** and pre-/post-HDB-37 points are not comparable. The closure is built offline by `scripts/bench/spb-sf256-build.sh`: single-process materialization needs ~3.8 kB per asserted triple and so does not fit 124 GiB at this scale, and the build slices the corpus instead — sound because the closure of a subset is contained in the closure of the whole (verified: 0 inferred triples lost). Serving it costs 115 B/triple (~50 GiB) and ~26 min to load, so the nightly waits on `/readyz`, not `/query` liveness. **Not yet cut over:** the corpus is built and both HornDB and GraphDB Free load and serve it, but the first `editorial-qps` reading is still missing — the smoke run took the bench host offline, most likely on memory, and HornDB does not enforce `max_query_memory`. Numbers and the open blocker: `docs/benchmarks.md`. |
