...
| MVCC with per-tuple visibility + delete path | **planned** | `SPEC-25` S1 ([#225](https://github.com/sunstoneinstitute/horndb/issues/225) in `TASKS.md`): begin/end visibility stamps on the tier commit clock, `retract_quad_batch` (the store cannot delete today), compaction, and the SPEC-24 S6 snapshot contract ([#215](https://github.com/sunstoneinstitute/horndb/issues/215)). Sits above the copy-on-write snapshot isolation row. |
