...
| Parallel chunked parsing in the bulk loaders | **implemented, on by default** | HDB-83. `loader/parallel.rs` plus `load_*_slice` beside each streaming loader: `oxttl`'s `split_slice_for_parallel_parsing` cuts the document into N chunks, one thread each, and the calling thread interns in document order so the parallel and serial paths produce identical stores (term ids included). N-Triples/N-Quads split unconditionally; Turtle is opt-in and additionally gated on `turtle_split_is_safe` (no `@base` anywhere — `oxttl` propagates leading prefixes into chunks but not the base — and no `@prefix` after the leading directive block), falling back to serial otherwise. **HDB-96 flipped `HORNDB_LOAD_THREADS` to `auto`**, resolved as `available_parallelism()` capped at 8; `=1` restores serial, an explicit `=<n>` is not capped, and Turtle *files* still need `HORNDB_PARALLEL_TURTLE=1` on top. Getting there took three corrections. The original HDB-83 rationale — parsing is ~13% of a load, ~80% sits in `Tier::insert_quad_batch` — was **overturned by HDB-85**: the tier is 12–14% and `parse` is the largest single phase (43.5% of a Turtle load). HDB-86 found why threads did not help: `parse_chunks_ordered` drained the chunk receivers in document order with only 16,384 triples buffered per chunk, so every producer but the one being drained parked on `send`. HDB-94 replaced that per-chunk constant with a **total in-flight budget in triples** (`HORNDB_LOAD_BUFFER_TRIPLES`, default 8,388,608), after which `parse` scaled 3.6×. The last blocker was HDB-83's tier regression (40.1s → 46.6s with threads), which HDB-84 removed by appending a batch as a run instead of rebuilding the partition. **HDB-96 then measured the sweep that had never been run** — 1/2/4/8/16 threads against `load_turtle_slice` / `load_ntriples_slice` into a real `Store` at trainmarks xlarge: Turtle 12.926s → **5.581s** (2.32×), N-Triples 9.672s → **4.903s** (1.97×), and the tier phases flat across the sweep (`group`+`build`+`merge_runs` 1.65s at 1 thread, 1.70s at 16), so the regression the serial default existed for does not reproduce. The cap is 8 because that is where the sweep flattens (the 16th thread is +1.5% on Turtle and −1.1% on N-Triples) and because `parse` is by then 14% / 3.6% of the load — what remains is interning and the tier, both serial by construction. Cost: peak RSS +78% Turtle / +57% N-Triples, almost all of it the 8M-triple parse buffer a one-chunk load never allocates. No contention from HDB-84's reader-blocks-writer path: the load is single-consumer and 153 runs against a 4,096 cap. Term ids are unchanged; `tests/parallel_loader.rs` compares interned ids across thread counts and budgets. Numbers and phase breakdown in `docs/benchmarks.md`. |
