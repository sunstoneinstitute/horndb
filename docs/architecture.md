...
| Valued closure / custom semirings (Sunstone annotated reasoning) | **partially implemented** | Readiness metrics **delivered** ([#11](https://github.com/sunstoneinstitute/horndb/issues/11)): `grb::ValuedMatrix` (FP64 `(max,×)` carrier, built-in + user-defined-op multiply) and `metrics::valued_transitive_closure` (N/nnz/density/iterations-to-fixpoint/per-iter frontier work/MxM share); `benches/valued_readiness.rs` measures valued-vs-boolean kernel split and the generic-kernel penalty, numbers + decision rule recorded in `BENCHMARKS.md`. **Measured outcome:** valued penalty ≤~3.6× (grows with result nnz) and the generic-kernel penalty for a scalar FP64 op is ~1.0× → built-in semirings suffice for Fork A; PreJIT only justified for a structured (Fork B) carrier. **Acceleration deferred** to [#12](https://github.com/sunstoneinstitute/horndb/issues/12): Fork A bench on the live GTIO/SKOS crosswalk → Fork B structured carrier → PreJIT, plus the SPEC-05 addendum. |
