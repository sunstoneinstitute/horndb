...
| trainmarks (DataTreehouse) end-to-end benchmark | **runnable** | All six trainmarks queries execute on `HornBackend` at all three scales (100K/1M/10M). Native driver `crates/bench-trainmarks` + `scripts/bench/trainmarks.sh`; numbers in `docs/benchmarks.md`. The q4 `OPTIONAL` cliff those numbers show was since fixed by the hash `LeftJoin` ([#116](https://github.com/sunstoneinstitute/horndb/issues/116), [#128](https://github.com/sunstoneinstitute/horndb/issues/128) Slice 2) — recorded table is a pre-fix baseline, re-run pending on `hornbench`. |
