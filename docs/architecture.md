...
| Named-graph query + update scoping (`GRAPH`, `FROM`/`FROM NAMED`) | **query implemented; update planned** ([#267](https://github.com/sunstoneinstitute/horndb/issues/267), `PLAN-28-04`) | `SPEC-28` phases 1–4. Phase 1 ([#264](https://github.com/sunstoneinstitute/horndb/issues/264)) refused `GRAPH` and the dataset clause instead of answering them wrongly; phase 3 replaced that refusal with real evaluation, so only the two `GRAPH ?g` shapes in the `GRAPH` row above still 400. Phase 2 ([#265](https://github.com/sunstoneinstitute/horndb/issues/265)) supplied the storage plumbing phase 3 reads through (`graph_len`, graph-scoped scan/iteration — see the SPEC-02 section above). Phase 3 ([#266](https://github.com/sunstoneinstitute/horndb/issues/266), `PLAN-28-03`) is done: `Algebra::Graph`, scan-scope lowering, `GRAPH ?g` as a scan column, `DatasetSpec`, the `union`/`strict` default-graph mode with its `[server.limits].default_graph` setting and per-query `default_graph` URL/form override, scope-aware pushdowns, path scoping, and the W3C `graph/` + `dataset/` families in `[sparql_query]`. `MemStore` grew a graph dimension in the same phase — a per-triple set of holding graphs beside the triple table, so every index stays triple-keyed (the work PLAN-28-04 shares). **Update is still default-graph-only** — `update.rs` rejects named-graph targets unless `SILENT` — and is phase 4's job. |
