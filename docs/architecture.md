...
| W3C SPARQL 1.1 *syntax* suite (query + update) | **implemented** | Suite key `sparql11-syntax` ([#110](https://github.com/sunstoneinstitute/horndb/issues/110), epic #10); `mf:*SyntaxTest11` types graded by `spargebra` (same parser as SPEC-07) via `TestKind::SparqlSyntax{Positive,Negative}`. 9 positive + 5 negative (5 update-form) curated checked-in cases under `tests/fixtures/sparql11-syntax/`; sub-ms, no network, no reasoner. |
