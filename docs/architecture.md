...
| `GRAPH` named-graph patterns | **refused (explicit 400; SPEC-28 phase 1)** | Was recorded as "implemented (Stage-1 merged-graph)", then as "broken — returns wrong answers"; both statuses are retired. `translate.rs::translate_pattern` now rejects `GraphPattern::Graph` (both the ground `GRAPH <g> { … }` and variable `GRAPH ?g { … }` forms), and `refuse_nonempty_dataset` rejects any non-empty `FROM`/`FROM NAMED` dataset clause — each returns `SparqlError::UnsupportedAlgebra`, which the server already maps to HTTP 400. A named-graph query now gets an explicit error instead of silent default-graph rows. `SPEC-28` phase 1 ([#264](https://github.com/sunstoneinstitute/horndb/issues/264)) is done; phases 2–4 still need to implement real named-graph evaluation. See `crates/sparql/INTEGRATION-NOTES.md`. ([#261](https://github.com/sunstoneinstitute/horndb/issues/261), parent epic; was #66/#7.) |
...
| Named-graph query + update scoping (`GRAPH`, `FROM`/`FROM NAMED`) | **planned** ([#264](https://github.com/sunstoneinstitute/horndb/issues/264)–[#267](https://github.com/sunstoneinstitute/horndb/issues/267), `PLAN-28-01`..`04`) | `SPEC-28` phases 1–4. Phase 1 ([#264](https://github.com/sunstoneinstitute/horndb/issues/264)) is done: `translate.rs::translate_pattern` refuses `GraphPattern::Graph` and any non-empty dataset clause with `SparqlError::UnsupportedAlgebra`, which the server returns as HTTP 400 — a named-graph query now gets an explicit error instead of default-graph rows. Update is at least as honest — `update.rs` rejects named-graph targets unless `SILENT`. Phase 2 ([#265](https://github.com/sunstoneinstitute/horndb/issues/265)) is also done, but it is storage-only plumbing (`graph_len`, graph-scoped scan/iteration — see the SPEC-02 section above) with no SPARQL-visible effect: `GRAPH`/`FROM` still 400 and Update still targets only the default graph. Phases 3–4 still need to add real `Algebra::Graph` evaluation, dataset construction, and named-graph update. |
