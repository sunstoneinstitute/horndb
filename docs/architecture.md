...
| Operator configuration system | **specified** | `SPEC-26` (approved) — layered config (built-in defaults < base `config.toml` < `config.d/*.toml` drop-ins < env < argv), live watch/reload (`notify` + `ArcSwap`), and a two-tier server-vs-query settings model with per-query URL-param overrides; wires `bind`, the `[simd]` knobs, `query_timeout`, `max_result_rows`, and `rdf12` to real config (`max_query_memory` parsed but enforcement delegated to a companion spec). New foundation crate `horndb-config`. Three phases (static config → query overrides + enforcement → live reload); first plan `PLAN-26-01`. No code yet; tracking issues filed at phase pickup. |
...
figure is superseded) is **materially improved** by #237 (galloping descent +
bulk leaf materialization: marginal 14.4 → 8.3 ns/tuple on hornbench); the
residual to ≤5 ns is the AoS→SoA input-column copy, tracked in
[#239](https://github.com/sunstoneinstitute/horndb/issues/239). The SPEC-02 NF2
