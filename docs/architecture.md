...
| HDT cold tier + tiering seam | **partially implemented** | `SPEC-25` S5 ([#229](https://github.com/sunstoneinstitute/horndb/issues/229) in `TASKS.md`). Done: `crates/storage/src/cold.rs` holds `ColdPartition`, an immutable memory-mapped subject-major encoding of one predicate, and `partition::Partition` is the `Warm`/`Cold` enum a graph maps a predicate to — so the seam is the partition, not the tier (a second `Tier` impl would have nothing to call; `Tier::predicate` has no live accessor). Demotion and promotion are by hand (`Store::demote` / `promote` / `demote_all`), swap at the same commit version like `compact()` does, and never land a write cold (a write promotes first). Measured 3.747 B/triple on the 40k-triple LUBM-shaped synthetic corpus. The mixed warm/cold conformance gate (acceptance #5) runs in CI: `HORNDB_COLD_TIER=1` makes every coarse write funnel in `HornBackend` call `Store::demote_all`, so the selected subset is graded twice more — once on the `VecTripleSource` read path, once with `HORNDB_DIRECT_SOURCE=1` — and both must report the warm step's counts exactly (565 pass / 0 fail / 146 skip). Only the `HornBackend`-backed suites (`sparql11-eval`, plus the `sparql11`/`sparql11-syntax` update cases) can change answer under it; `owl2`, `owl2-w3c-rl`, `rdf12-n-triples` and `sssom-mappings` run on `horndb_owlrl::Engine`, which keeps facts in its own `MemStore` and never touches `horndb_storage::Store`, so they are tier-independent by construction — putting them on a storage-backed reasoner is a SPEC-04 change, not an S5 one. `storage_tier_bytes_estimated{tier}` now reports real per-tier values (the [#148](https://github.com/sunstoneinstitute/horndb/issues/148) deferral, closed by SPEC-25 S5 HDB-178): `TierStats` carries `bytes_cold` (the summed mapped length of cold partitions) alongside the `bytes_estimated` warm+cold total, and the collector emits `tier="dram"` (warm) and `tier="cold"` series instead of the old placeholder `tier="unknown"`. Still planned: placement policy and access statistics with the SPEC-08 `HotSetAdvisor` bias, and durable cold placement across `Store::open` (which today deletes `<dir>/cold` and replays every partition warm). CXL/NVMe placement stays SPEC-09 (Stage 3). |
...
The bilinear-join runtime (S7) is **partially implemented**: per-predicate leaf
extents, the `HashJoinRule` kernel, and cost-based leaf ordering have landed
(see the bilinear rule operator row below). SPEC-06 F4 — "the planner picks the
decomposition to minimise intermediate Z-set size" — is **implemented as a
planner but not yet on the engine path**: `NaryPlan::from_body` costs a body
over the SPEC-23 `Stats` seam, but only tests call it. Nothing supplies
`&dyn Stats` at rule-registration time yet, so registered rules are still
planned in the order their joins are pushed; that wiring is the E4 seam
([#188](https://github.com/sunstoneinstitute/horndb/issues/188)). A sort-merge
kernel and codegen emitting kernel instances are still planned. The intra-tick joint
fixpoint (S8) remains **planned** under `SPEC-24`
...
| Bilinear rule operator (two-pattern bodies) | **implemented** | `operator.rs`, `circuit.rs`. `NaryPlan` binds each join leaf to a per-predicate slice of the base extent (`PredExtent`, `SPEC-24` S7) instead of scanning the whole extent on every join — a rule's `body_predicates()` declares which predicate each leaf reads, `None` for a leaf with a variable in predicate position (e.g. prp-dom). A rule author instantiates `kernels::HashJoinRule` (`kernels.rs`, SPEC-24 S7) instead of hand-writing a nested loop: it is parameterised by two body triple patterns and a head pattern, hash-joins on their shared variables, and is correct for arbitrary Z-set multiplicities; `tests/bilinear_correctness.rs` pins it against a nested-loop reference with a proptest. **Leaf order is cost-chosen** (SPEC-06 F4, `SPEC-24` S7): `NaryPlan::from_body(id, body, head, stats)` takes the rule body as triple patterns and greedily builds a connected left-deep order — start from the pattern with the smallest `StatsEstimator::estimate_pattern`, then repeatedly append the unused pattern that shares a variable with the prefix and minimises `estimate_bgp(prefix ∪ {p})`, ties by declaration order. Each level emits a `HashJoinRule` whose head is the final head (last level) or the three-slot projection of the prefix's live variables. Uninformed statistics (`ZeroStats::is_informed() == false`) keep declaration order, mirroring `horndb_wcoj::planner`; a disconnected body (cross product) and a prefix needing a fourth live variable are errors. `NaryPlan::leaf_order()` exposes the choice. **Callers so far are tests only** —
supplying statistics at rule-registration time is the E4 seam
([#188](https://github.com/sunstoneinstitute/horndb/issues/188)), so the engine
does not cost-order its rules yet. Tests: `tests/nary_plan.rs::cost_ordering` — the selective leaf goes first on a skewed extent and `apply_full` matches the declaration-order plan's row for row. A sort-merge kernel and codegen emitting kernel instances from `rules.toml` remain **planned** (`SPEC-24` S7, [#216](https://github.com/sunstoneinstitute/horndb/issues/216)). |
...
| `GRAPH` named-graph patterns + dataset clause (`FROM`/`FROM NAMED`) | **implemented** (SPEC-28 phase 3 + its S3 amendment) | Earlier statuses — "implemented (Stage-1 merged-graph)", "broken — returns wrong answers", "refused (explicit 400)", "implemented except two families of `GRAPH ?g`" — are all retired. `translate.rs` builds `Algebra::Graph`; `plan/lower.rs` pushes the scope onto every scan leaf under it (`GraphScope`, innermost `GRAPH` wins), so a ground `GRAPH <g>` scans only `g` and an unknown graph IRI gives zero rows, never an error. `GRAPH ?g` is one **`PerGraph` plan node** (SPEC-28 D6 as amended by S3, HDB-74): it evaluates the whole block once per named graph with `?g` free, then joins `{?g → that graph}` onto that graph's rows — SPARQL 1.1 §18.2.2.2 as written. The loop lives in the operator (`exec::op::per_graph`), so plan size stays independent of the graph count, and `?g` never binds the default graph. `FROM`/`FROM NAMED` build a `DatasetSpec`, including `FROM NAMED` with no `FROM` = empty default graph. With no dataset clause the default graph follows `[server.limits].default_graph`: `union` (default) = every non-reserved graph, deduped; `strict` = the default-graph sentinel only. Graphs under `https://horndb.io/graph/` are reserved: out of the union and out of `GRAPH ?g` enumeration unless named explicitly. Property paths inherit the scope *before* the closure runs. Count/group-count pushdowns are scope-aware or decline (`Ok(None)`), so no shortcut can answer a scoped query with a whole-store count; estimates stay coarse and never reach a result. **No shape of `GRAPH ?g` is refused any more.** The two families phase 3 rejected both answer: (1) a barrier between the wrapper and its scan leaves — a sub-`SELECT`, `DISTINCT`, `GROUP BY`, `LIMIT`, a property path, a nested `GRAPH`, a `VALUES` arm — because the block is evaluated whole, per graph, rather than carrying a graph column up from the leaves; (2) a block that **reads** `?g` (`FILTER`, `BIND`, an `OPTIONAL` condition, `?g` in an `OPTIONAL`'s right arm), because `?g` is genuinely free inside the block and arrives only in the post-join. This turned W3C `graph-variable-scope` and `graph-optional` green, along with the six `sparql11-eval` cases HDB-135 tracked (`subquery01`..`subquery05`, `property-path/pp35`). One consequence is deliberate: under `GRAPH ?g` the count/group-count pushdown no longer fires, because a sum over per-graph counts is not one scoped count. Conformance: W3C `graph/` + `dataset/` families, 26 of 29 cases selected and green on both backends (`harness/selected.toml` `[sparql_query]`); the other 3 (blank-node grading) in `harness/KNOWN-MANIFEST-BUGS.md`. See `crates/sparql/INTEGRATION-NOTES.md`. ([#266](https://github.com/sunstoneinstitute/horndb/issues/266); [#261](https://github.com/sunstoneinstitute/horndb/issues/261) parent epic; was #66/#7.) |
