...
| `GRAPH` named-graph patterns | **broken — returns wrong answers** | Was recorded as "implemented (Stage-1 merged-graph)"; that status was misleading and is retired. `translate.rs::translate_pattern` **discards** the `GRAPH` wrapper (`GraphPattern::Graph { name: _, inner }`) and all four `translate_query_with` arms bind `dataset: _`, so `GRAPH <g> { … }`, `GRAPH ?g { … }`, `FROM`, and `FROM NAMED` all evaluate against the default graph and return HTTP 200. On a store whose data lives in named graphs these are wrong answers, not missing features, and a caller cannot detect them. `SPEC-28` phase 1 makes them explicit 400s; phases 2–4 implement them. See `crates/sparql/INTEGRATION-NOTES.md`. ([#261](https://github.com/sunstoneinstitute/horndb/issues/261); was #66/#7.) |
...
| Named-graph query + update scoping (`GRAPH`, `FROM`/`FROM NAMED`) | **planned** ([#264](https://github.com/sunstoneinstitute/horndb/issues/264)–[#267](https://github.com/sunstoneinstitute/horndb/issues/267), `PLAN-28-01`..`04`) | `SPEC-28` phases 1–4. **Today's behaviour is a silent wrong answer:** `translate.rs::translate_pattern` discards the `GRAPH` wrapper (`GraphPattern::Graph { name: _, inner }`) and all four `translate_query_with` arms bind `dataset: _`, so a named-graph query returns default-graph rows with HTTP 200. Update is at least honest — `update.rs` rejects named-graph targets unless `SILENT`. Phase 1 turns the query-side silence into an explicit 400 before any real support lands. |
