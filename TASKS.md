...
- [ ] **HIGH** · _Performance_ — SPEC-12 SIMD layer: `horndb-simd` primitives crate **landed** (F4+F5); WCOJ seek/intersect consumer (F1) **landed** — `VecIter` SoA-column + `PackedColumn` block-finish seek through `horndb_simd::lower_bound`, `LeapfrogJoin` k==2 `horndb_simd::intersect` fast path, real `per_tuple` microbench wired (differential fuzzer + leapfrog oracle green). **Remaining:** record `per_tuple` ns/tuple on hornbench (≤2.5 target); wire the SIMD intersect into `BatchIter`'s inlined leapfrog (the production executor path); storage `rdf:type` scan (F2) + delta-apply (F3) consumers ([#132](https://github.com/sunstoneinstitute/horndb/issues/132))
