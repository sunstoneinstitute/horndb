...
- [v] **MEDIUM** · _Performance_ — Valued-closure / custom-semiring acceleration ([#12](https://github.com/sunstoneinstitute/horndb/issues/12)) — _wip: 122d0f80@Stigs-MacBook-Pro.local · task-12-valued-closure-fork-a · 2026-06-18T13:31:57Z_
...
- [x] **SPEC-07 SPARQL.** ([#7](https://github.com/sunstoneinstitute/horndb/issues/7))
...
- [x] **SPEC-08 ML.** ([#8](https://github.com/sunstoneinstitute/horndb/issues/8))
  Epic complete — the ML HTTP boundary is delivered; the remainder is a
  Stage-2 increment (native-linkage heavy).
  Delivered: the HTTP boundary behind an off-by-default `server` feature —
  `POST /nl-query` (F3, LLM→SPARQL via the `Translator` trait, generated SPARQL
  always returned for audit, LLM never bundled / mock-tested hermetic),
  `GET /ml-audit` (F6, paginated + since-filtered, bounded per-request work),
  cost reporting (`CostReport`), and training-data leakage controls
  (`config::LlmPrivacy`, boundary-enforced on every channel: success
  explanation, error body, question log) ([#105](https://github.com/sunstoneinstitute/horndb/issues/105), PR #106).
  The traits + in-process scaffolding (`CandidateGenerator`, `PlanAdvisor`,
  `HotSetAdvisor`, provenance, registry) shipped in Stage 1.
  Deferred to Stage-2 (re-file as an increment under #8 when pulled in): the
  real FAISS-backed `CandidateGenerator` + its synthetic entity-resolution
  acceptance (≥10× speedup over brute force, ≥99% false-positive rejection by
  symbolic re-verification). Native FAISS vendoring/linkage (akin to the
  GraphBLAS build) is separable from the HTTP boundary and not on any
  correctness path (SPEC-08 candidate generation is opt-in / human-review by
  default).

- [x] **SPEC-10 rdflib-compatible Python API.** ([#9](https://github.com/sunstoneinstitute/horndb/issues/9))
...
- [x] **SPEC-01 harness.** ([#10](https://github.com/sunstoneinstitute/horndb/issues/10))
...
- [x] **Closure valued-reasoning readiness metrics.** ([#11](https://github.com/sunstoneinstitute/horndb/issues/11))
...
- [v] **Valued-closure / custom-semiring acceleration.** ([#12](https://github.com/sunstoneinstitute/horndb/issues/12))
...
- [ ] **Speed up integration test runs.** ([#108](https://github.com/sunstoneinstitute/horndb/issues/108))
  ...
  single regression. On macOS it compounds: each fresh test binary triggers a
  Gatekeeper + XProtect scan on first exec (mitigated per-dev via Developer Tools —
  see AGENTS.md — but that doesn't cut link/run time). Levers to evaluate, not
  pre-decided: **parallelize** (`cargo nextest`, runs across binaries concurrently,
  no source changes) and/or **consolidate** small `tests/*.rs` into fewer targets
  (fewer link steps + fewer binaries to scan). Done-when: measured before/after on
