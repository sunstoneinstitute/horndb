...
- [x] **CRITICAL** · _Completeness_ — SPEC-24 S4: engine wiring — SPARQL Update → Circuit → readers — after #212 ([#213](https://github.com/sunstoneinstitute/horndb/issues/213))
...
- [x] **HIGH** · _Operational_ — SPEC-25 S3: write-ahead log + crash recovery — after #225/#226; SPEC-24 S5 #214 layers on this format ([#227](https://github.com/sunstoneinstitute/horndb/issues/227))
...
- [x] **HIGH** · _Performance_ — SPEC-04: genuine delta-driven semi-naïve firing for the compiled rules ([#134](https://github.com/sunstoneinstitute/horndb/issues/134))
...
- [ ] **LOW** · _Maintainability_ — Extract shared `compile_bgp_patterns` helper in `crates/sparql/src/exec/horn.rs` (lode HDB-81, no GitHub issue)
...
- [x] **EPIC E3 — SPEC-02 storage Stage-2.** _HIGH · Completeness._
  ...
  **Done 2026-07-19** (PR [#231](https://github.com/sunstoneinstitute/horndb/pull/231)): `SPEC-25` approved; decomposed into
  leaf issues [#224](https://github.com/sunstoneinstitute/horndb/issues/224)–[#230](https://github.com/sunstoneinstitute/horndb/issues/230) — tracked as the SPEC-25 phase tasks in this file (S1 [#225](https://github.com/sunstoneinstitute/horndb/issues/225),
  S2 [#226](https://github.com/sunstoneinstitute/horndb/issues/226), S4 [#228](https://github.com/sunstoneinstitute/horndb/issues/228), and S6 [#230](https://github.com/sunstoneinstitute/horndb/issues/230) are immediately pickable).
...
- [x] **SPEC-03 NF1: close `per_tuple` hot-path overhead.** ([#237](https://github.com/sunstoneinstitute/horndb/issues/237))
  Measured ~67 ns/tuple on hornbench (2026-06-30) — ~an order of magnitude over the
  SPEC-03 NF1 target (≤5 ns/tuple; #132 tracked a stricter ≤2.5 — reconcile against
  SPEC-03 as source of truth). Wiring `horndb_simd::intersect` left it unchanged
  (criterion A/B "no change"): the bottleneck is the depth-1 narrow-run leapfrog +
  Arrow materialization, **not** the intersect kernel — so this is WCOJ / SPARQL-runtime
  work, not SIMD. Spun out of #132 (SPEC-12), whose SIMD consumers (F1/F2) landed.
  Levers: batch- vs per-tuple materialization (cf. #143), skip trie setup below the
  WCOJ cutover (cf. #204). Gate: `benches/per_tuple.rs` at the NF1 target on hornbench
  (`docs/benchmarks.md`), `four_cycle` no-regress. Spec: `docs/specs/SPEC-03-query-engine.md` NF1.
...
- [x] **SPEC-23 Phase 2: heuristic rewrite passes.** ([#202](https://github.com/sunstoneinstitute/horndb/issues/202))
...
- [x] **SPEC-24 S1: delta-incremental rule retraction.** ([#210](https://github.com/sunstoneinstitute/horndb/issues/210))
...
- [x] **SPEC-24 S2: delta-incremental closure retraction + exact seeded retraction.** ([#211](https://github.com/sunstoneinstitute/horndb/issues/211))
...
- [x] **SPEC-24 S4: engine wiring — SPARQL Update → Circuit → readers.** ([#213](https://github.com/sunstoneinstitute/horndb/issues/213))
...
- [x] **SPEC-25 S1: per-tuple MVCC visibility + delete path.** ([#225](https://github.com/sunstoneinstitute/horndb/issues/225))
  ...
  with batch atomicity, version-consistent reads across all six orderings +
  Roaring sets + snapshot export, and compaction that respects pinned
  snapshots; satisfy the SPEC-24 S6 snapshot contract (#215), agreeing the
  version-clock binding with E2 before building. Retires the sparql overlay.
  Spec: `docs/specs/SPEC-25-storage-stage2.md` §S1. Plan: `PLAN-25-MM` when
...
- [x] **SPEC-25 S2: persistent on-disk dictionary.** ([#226](https://github.com/sunstoneinstitute/horndb/issues/226))
  ...
  id→term in O(1) probes and term→id in O(term length) on the mapped base;
  NF3 stays in force for the overlay and inline terms. A reopened store
  resolves both directions without re-interning the corpus. Spec §S2.
  Gate: SPEC-25 acceptance #2.
...
- [x] **SPEC-25 S3: write-ahead log + crash recovery.** ([#227](https://github.com/sunstoneinstitute/horndb/issues/227))
  ...
  idempotent replay, bit-identical modulo timestamps; torn-tail truncation;
  checkpoint records its commit version and truncates the log. Upgrades
  SPEC-02 NF5 for WAL-opened stores. On-disk format owned here; SPEC-24 S5
...
- [x] **SPEC-26 Phase 2: query-scoped overrides + enforcement.** ([#251](https://github.com/sunstoneinstitute/horndb/issues/251))
...
- [ ] **SPEC-25 S6: deferred Stage-1 acceptance benches on hornbench.** ([#230](https://github.com/sunstoneinstitute/horndb/issues/230))
  Run and record SPEC-02 acceptance 2/3/4 at the scale the criteria name:
  LUBM-8000 import ≤30 min, fully-warm footprint ≤55 GB, `rdf:type` scan
  ≥80% of STREAM Triad (the 2026-07-07 `partition_scan` ~104%-of-Triad number
  is kernel evidence, not the corpus-scale measurement). Fill
  `crates/storage/STAGE1-ACCEPTANCE.md` rows 2/3/4 + `docs/benchmarks.md` in
  the same change; honesty clause applies (hornbench ≠ reference workstation;
...
- [x] **SPEC-03 NF1: columnar (SoA) `VecTripleSource`.** ([#239](https://github.com/sunstoneinstitute/horndb/issues/239))
  ...
  8.3 ns/tuple on hornbench) shows the residual is the AoS→SoA input-column copy:
  `LevelColumn::from_aos` 46% self, `VecIter::active_run` (incl. `distinct_run`) 33%,
  `scalar::intersect` 15%. SoA storage drops `from_aos` on this path — `active_run`
  returns `&col[lo..hi]` in place, satisfying SPEC-03 **NF2** ("no copy of input
  columns") — and makes the seek / `open_level` column accesses contiguous. Adapt the
  `sorted_rows` consumer (`SnapshotStats` in `horndb-sparql`); keep `distinct_run` for
  inner levels (the leaf run is already distinct). Gate: `benches/per_tuple.rs`
  `wide_4x100k` ≤5 ns/tuple on hornbench, `two_star_50k` + `four_cycle` no-regress,
...
- [ ] **SPEC-25 S5: HDT cold tier + tiering seam.** ([#229](https://github.com/sunstoneinstitute/horndb/issues/229))
  ...
  ≤6 B/triple, ≤2× read amplification — NF1/NF4); demotion/promotion at
  predicate-partition granularity from access stats with the SPEC-08
  `HotSetAdvisor` bias alongside (never instead of) them; writes never land
  cold; only visibility-settled tuples demote (compaction product of S1).
  Delivers the #148 deferral: real `storage_tier_bytes_estimated{tier}`
...
- [ ] **Extract shared `compile_bgp_patterns` helper in `crates/sparql/src/exec/horn.rs`.**
  (tracked as lode HDB-81; no GitHub issue)
