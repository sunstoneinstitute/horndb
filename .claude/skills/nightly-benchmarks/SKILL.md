...
`spb-sf128.nt` (no reasoning) over SPARQL/HTTP and drives it with the LDBC SPB driver,
...
> `aggregation-qps` was the headline. Since it, the corpus is the real SF=0.128
> one (~233 M closure triples) and two editorial agents run alongside four
...
> ~250x bigger and readers now share the store with writers. Compare only
...
- Scenario/driver assets are a prepared tree at `$SPB_ASSETS` on the runner; the dataset
  is `spb-sf128.nt` (a symlink into `/home/bench/horndb-bench/spb-sf128/`, built by
  `scripts/bench/spb-scale-build.sh`; earlier corpora are still beside it).
  The driver also reads `$SPB_ASSETS/generated-sf128` — `dataset.info` plus the
...
- **Oxigraph precondition:** the persisted RocksDB stores (`spb-store` and the
  ...
  one-time-bootstrap model as GraphDB. Both the Oxigraph stores and the
  GraphDB `spb` repo are rebuilt at a new scale by `scripts/bench/spb-scale-bootstrap-engines.sh`,
